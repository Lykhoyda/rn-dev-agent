...
| `METRO_START_UNAVAILABLE` with `launcher alive at deadline; readiness deadline N ms expired` from the package script | the refusal's own `Metro log tail` (a pre-shutdown snapshot) | Metro was still starting (cold per-session cache, loaded host) when the 90 s readiness budget ran out; anything Metro printed while being stopped, such as an `EPIPE`, is a consequence, not the cause | Retry; for a project that needs longer set `.rn-agent/config.json` → `{ "metro": { "readinessTimeoutMs": 150000 } }` (integer, 1000–600000; a timeout only, a malformed value refuses with `METRO_READINESS_TIMEOUT_INVALID`). The generated adapter derives its `ensure-metro` SIGKILL bound from that budget plus 100 s of headroom for the work outside the readiness loop, so 150000 is actually waited. The first-build advisory `no-install-receipt` is expected, not a second failure |
