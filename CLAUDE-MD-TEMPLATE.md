...
| `KEYBOARD_DISMISS_FAILED` refusal (iOS) | `device_snapshot` | A visible keyboard could not be proven hidden after native swipe/control and the optional injected JS tier, so no tap was performed | Connect CDP (`cdp_status`) so the JS tier can run, dismiss the keyboard explicitly, then retry with a fresh snapshot/ref |
