...
- **`HELPERS_VERSION`** (`injected-helpers.ts:5`): a **single branch-level bump is sufficient** — Task 1 does 26→27; later surface-changing tasks may keep 27 (one value > 26 forces re-injection on merge; per-task bumps are optional traceability, not required). The #321 version guards assert the **≥26 baseline** (value-agnostic), so any bump count is safe.
