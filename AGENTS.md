...
- The registered `cdp_connect` tool is `createRegisteredConnectHandler`
  ...
  by contract tests.
- Park-state action replay (GH #628): `detectEntryDeclaration` in
  `src/domain/reusable-action.ts` owns bounded pre-body declaration detection;
  `src/domain/park-entry.ts` owns shared admission, parked-body invariants, and
  anchor derivation. `cdp_run_action` runs the read-only preflight through
  `src/tools/park-probe.ts` and refuses `PARK_STATE_MISSING` before any step.
  Refuse-not-strip and default-cold compatibility are captain decisions — never
  auto-suppress a launch prologue or auto-classify legacy actions as parked.
