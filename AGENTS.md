...
Phase 1 (the cut) is done: three packages exist, the session, MCP, observability
and host-bundle stack is gone, and the kept core identifiers renamed once.
`qaren check`, Jev, blocks, recovery, `qaren pr`, `qaren listen` and packaging
arrive in Phases 2 to 8.
...
- Root workspace: Yarn 4 workspace, managed by `package.json` and `yarn.lock`.
  ...
  with `cargo` directly.
- `packages/qaren-cli/`: Rust CLI, package and binary `qaren`, `publish = false`.
  Copied from the `rn-qa` prototype; today it has the prototype verbs
  (`prepare`, `prewarm`, `status`, `complete`, `cleanup`) and the adapters,
  build plan, candidate drift, run records, receipts, failure taxonomy
  (`src/failure.rs`) and secret redaction (`src/redact.rs`). The crate still
  carries the prototype's own names (`rn_qa` lib, `rn-qa` usage text,
  `RN_QA_LOCK_ROOT`, `~/.rn-qa/locks`, `.rn-qa` run dirs, the `rn-qa/1`
  scenario schema); Phase 2 renames them when it rewrites the crate. `observe/`
  is the Observe SPA (Vite), built by `corepack yarn build:observe`;
  `target/` and `observe/dist/` are ignored.
- `packages/qaren-core/`: TypeScript screen layer the CLI will spawn as a
  child. Today it holds the kept handlers (`src/handlers/`), the CDP client and
  injected helpers, the native runner clients (`src/runners/`), the
  learned-action domain (`src/domain/`), lifecycle utilities and `src/util/`.
  `dist/` is generated by `corepack yarn build:core` and not committed. The
  package has no `bin`; its CLI entries run as `node packages/qaren-core/dist/<entry>.js`.
  It is not published; the CLI tarball bundles it (Phase 8).
...
- In `packages/qaren-core` the names are final and have no alias: `QAREN_*`
  ...
  Old names that remain on purpose: the Rust prototype names listed in the map
  (Phase 2), the plugin skill text (Phase 8), the GitHub slug
...
- The native runner clients still require `QAREN_SESSION_ID` and
  `QAREN_CLAIM_EPOCH` before a launch until the CLI lease replaces them.
...
node --test scripts/test/check-document-ownership.test.ts
