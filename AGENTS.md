...
- `packages/claude-plugin/`: the ONE directory both marketplaces install
  (GH #892). Owns Claude manifests, hooks, commands, agents, skills, templates,
  helper scripts, packaged native runners, runner manifest, the single bundled
  core runtime (`rn-dev-agent-core/`, host-neutral metadata), and the generated
  Codex adapters: `.codex-plugin/plugin.json`, `codex.mcp.json`, `bin/`,
  `codex-skills/`, `codex-commands/`, `codex-agents/`, `codex-templates/`,
  `AGENTS-MD-TEMPLATE.md`. Never hand-edit a generated `codex-*`/`bin/` path.
...
- Distributed plugin package for BOTH hosts: `packages/claude-plugin/`.
