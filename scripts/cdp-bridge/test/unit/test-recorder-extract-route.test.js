    ...
    routes: [
      {
        ...
        state: {
          index: 2,
          routes: [{ name: 'Home' }, { name: 'Notifications' }, { name: 'Tasks' }],
        },
      },
    ],
  };
...
test('returns null when index exceeds routes length (defensive)', () => {
  assert.equal(
    extractActiveRouteForTest({ index: 5, routes: [{ name: 'A' }, { name: 'B' }] }),
    null,
  );
});
...
test('defensive: bails after 20 levels of nesting (no infinite loop on self-reference)', () => {
  ...
  let result;
  assert.doesNotThrow(() => {
    result = extractActiveRouteForTest(state);
  });
...
test('B135 regression guard: in-IIFE source matches TS mirror (function shape + semantics)', () => {
  ...
  return import('../../dist/cdp/test-recorder-helpers.js').then((mod) => {
    ...
    assert.ok(
      src.includes("typeof s.routeName === 'string'"),
      'B135 drift: in-IIFE extractActiveRoute must handle plugin routeName shape',
    );
    assert.ok(
      src.includes('Array.isArray(s.routes)'),
    ...
    assert.ok(
      src.includes("typeof s.nested === 'object'"),
    ...
    assert.ok(/depth\s*<\s*20/.test(src), 'B135 drift: depth guard must remain at 20 levels');
  });
});
