...
test('#210 screenshot-int: runner THROWS (down) + raw succeeds → simctl fallback, no crash', async () => {
  _setRunAgentDeviceForTest(async () => { throw new Error('rn-fast-runner not started'); });
  setRawForTest({ iosResolver: async () => 'UDID-TEST', iosCapturer: async () => true });
  try {
    const res = await captureAndResizeScreenshot({ platform: 'ios', platformExplicit: false, maxWidth: 0 });
    assert.ok(!res.isError, 'a thrown runner error must be caught and routed to simctl');
    assert.equal(parse(res).data.via, 'simctl');
  } finally { _resetRunAgentDeviceForTest(); resetRawForTest(); }
});

test('#210 screenshot-int: flow active → runner fn is NEVER called (raw-only)', async () => {
  let runnerCalls = 0;
  _setRunAgentDeviceForTest(async () => { runnerCalls++; return { content: [] }; });
  setRawForTest({ iosResolver: async () => 'UDID-TEST', iosCapturer: async () => true });
  ...
  } finally { if (lease.ok) arbiter.release(lease.lease); _resetRunAgentDeviceForTest(); resetRawForTest(); }
});
...
test('#210 screenshot-int: android raw fallback reports via:adb (accurate backend name)', async () => {
  _setRunAgentDeviceForTest(async () => { throw new Error('runner down'); });
  setRawForTest({ androidResolver: async () => 'emulator-test', androidCapturer: async () => true });
  try {
    const res = await captureAndResizeScreenshot({ platform: 'android', platformExplicit: false, maxWidth: 0 });
    assert.ok(!res.isError);
