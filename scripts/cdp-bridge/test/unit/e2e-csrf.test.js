...
const post = (over = {}) => ({
  method: 'POST',
  headers: { 'content-type': 'application/json', 'x-csrf-token': T, ...over.headers },
  ...over,
});

test('makeCsrfToken returns a long unguessable hex token, unique per call', () => {
  const a = makeCsrfToken();
  const b = makeCsrfToken();
...
test('missing/wrong csrf is refused (403)', () => {
  ...
  assert.equal(
    isPostAllowed({ method: 'POST', headers: { 'content-type': 'application/json' } }, T).status,
    403,
  );
});
