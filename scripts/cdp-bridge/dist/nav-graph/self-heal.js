...
export function getChangedNavFiles(fromCommit, projectRoot) {
    if (!/^[0-9a-f]{4,40}$/i.test(fromCommit))
        return null;
...
export function stampGraphWithCommit(projectRoot) {
    const graph = readGraph(projectRoot);
    if (!graph)
        return;
...
export function buildSelfHealAdvice(failedScreen, failedMethod, platform) {
    ...
    if (platform === 'android') {
        ...
        if (tabEntry)
            advice.push(`Android note: ${tabEntry.reason}`);
    }
    if (platform === 'ios') {
        const gearEntry = getPlaybookForContext('tap_near_gear_icon', 'ios');
        if (gearEntry)
            advice.push(`iOS note: ${gearEntry.reason}`);
    }
    return {
