 ...
 */
export function scanTelemetry(): TelemetryEvent[] {
 ...
 */
export function groupFailures(events: TelemetryEvent[]): FailureStats[] {
  const groups = new Map<string, FailureStats>();

  for (const event of events) {
    if (event.event !== 'tool_call' && event.event !== 'ghost_attempt') continue;
    ...
    if (!isFail && !isGhostRecovery) continue;
...
export function generateCandidates(
  ...
  const candidates: CandidateHeuristic[] = [];
  let nextId = 1;
  ...
  for (const group of stats) {
    if (group.total < MIN_OCCURRENCES) continue;
...
function extractEnvFilter(env: EnvironmentFingerprint): Partial<EnvironmentFingerprint> {
  const filter: Partial<EnvironmentFingerprint> = {};
  if (env.platform) filter.platform = env.platform;
  if (env.engine) filter.engine = env.engine;
  if (env.rn_version) {
...
export function computeDecay(
  heuristics: Array<{ id: string; confidence: number }>,
  events: TelemetryEvent[],
): { decayed: string[]; removed: string[]; currentConfidence: Map<string, number> } {
  ...
  for (const event of events) {
    if (event.family_id) activeFamilies.add(event.family_id);
    if (event.tool) activeTools.add(event.tool);
  }

  const decayed: string[] = [];
  const removed: string[] = [];
  const currentConfidence = new Map<string, number>();
