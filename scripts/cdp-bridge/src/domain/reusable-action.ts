...
export interface M7Metadata {
   ...
   * hybrid composition: when the user's task requires a state that the
   * current app doesn't satisfy, the agent scans for an action whose
   * `produces` covers the gap and replays it as a deterministic prologue
   * before continuing with interactive tools.
   *
   * Optional — actions without `produces` continue to work; the agent
   * falls back to intent-string matching for them.
   *
   * v1 supports primitive values only (string | number | boolean). The
   * inline YAML serialization rules out commas + newlines inside values.
   */
...
/**
 * Parse the inline `produces` map: `{ key: value, key: value }`. Values
 * are typed as boolean (`true`/`false`), number (digits + optional dot
 * + optional sign), or string (everything else, with surrounding
 ...
 * inside values are not supported in v1 (D1209).
 */
