...
trackedTool(
  'cdp_nav_graph',
  'Navigation graph tool with 3 actions. scan: extract full topology from running app (all screens + navigators including unvisited), persist to .rn-nav-graph.yaml. read: load cached graph from disk (no CDP). navigate: plan multi-step path to a target screen with method ranking and prerequisite detection. Use cdp_navigation_state for current route only.',
  {
