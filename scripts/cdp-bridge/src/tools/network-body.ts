...
export function createNetworkBodyHandler(getClient: () => CDPClient) {
  return withConnection(getClient, async (args: { requestId: string; maxLength?: number }, client) => {
    ...
    if (client.networkMode !== 'cdp') {
      return failResult(
        'Network.getResponseBody requires CDP network mode (RN 0.83+). ' +
        'Current mode: ' + client.networkMode,
        { hint: 'The hook-based fallback does not capture response bodies.' },
      );
    }
    ...
    try {
      ...
      if (msg.includes('No resource with given identifier') || msg.includes('No data found')) {
        return failResult(
          `Response body not available for ${args.requestId}. The response may not have finished loading, or the body was not retained by the engine.`,
