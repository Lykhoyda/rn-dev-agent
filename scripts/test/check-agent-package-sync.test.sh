...
write_valid_repo() {
  ...
  printf '%s\n' 'console.log("workflow-check fixture")' > "$tmp/packages/rn-dev-agent-core/dist/workflow-check.js"
  ...
  for skill in capturing-proof creating-actions rn-best-practices rn-debugging rn-device-control rn-feature-development rn-setup rn-testing rn-workflow sending-feedback using-rn-dev-agent; do
  ...
  for command in build-and-test check-env check-vercel-rules debug-screen doctor list-learned-actions lock-e2e nav-graph observe proof-capture rn-feature-dev run-action run-workflow send-feedback setup test-feature; do
