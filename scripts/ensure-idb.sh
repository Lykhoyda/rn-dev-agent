...
#   brew install idb-companion   (the macOS daemon)
...
if ! command -v brew >/dev/null 2>&1; then
  ...
  echo "Install manually: brew install idb-companion && pipx install fb-idb"
  exit 0
fi
...
if [ -f "$MARKER" ]; then
  ...
  if [ "$LAST_STATUS" = "failed" ] && [ -n "${LAST_TS:-}" ] && [ $((NOW - LAST_TS)) -lt "$BACKOFF_SECS" ]; then
    echo "idb install failed recently — retrying after backoff (manual: brew install idb-companion && pipx install fb-idb, log: $LOG)"
    exit 0
  fi
fi
...
if [ "${RN_AGENT_IDB_DRY_SPAWN:-}" = "1" ]; then
  ...
  echo "idb missing — installing in background (brew install idb-companion && pipx install fb-idb). Log: $LOG"
  exit 0
fi
