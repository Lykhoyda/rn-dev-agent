...
expect_codex_skill_inventory() {
  ...
  while IFS= read -r name; do
    ...
    for files in SKILL.md agents/openai.yaml; do
      expect_file "packages/codex-plugin/skills/$name/$files"
    done
    if ! grep -q 'GENERATED by scripts/build-host-runtimes.ts' "$ROOT/packages/codex-plugin/skills/$name/SKILL.md"; then
      fail "Codex workflow skill $name must be marker-owned generated output"
    fi
    if ! grep -Eq 'allow_implicit_invocation:[[:space:]]+false' "$ROOT/packages/codex-plugin/skills/$name/agents/openai.yaml"; then
...
for path in \
  ...
  packages/codex-plugin/scripts/snapshot_state.sh \
...
for helper in expo_ensure_running.sh eas_resolve_artifact.sh check-vercel-rules.mjs snapshot_state.sh; do
  if ! cmp -s "$ROOT/scripts/$helper" "$ROOT/packages/codex-plugin/scripts/$helper"; then
    fail "Codex package scripts/$helper must match scripts/$helper"
  fi
  if ! cmp -s "$ROOT/scripts/$helper" "$ROOT/packages/claude-plugin/scripts/$helper"; then
    fail "Claude package scripts/$helper must match scripts/$helper"
  fi
done
if ! cmp -s "$ROOT/packages/codex-plugin/src/AGENTS-MD-TEMPLATE.md" "$ROOT/packages/codex-plugin/AGENTS-MD-TEMPLATE.md"; then
  fail "Codex package AGENTS-MD-TEMPLATE.md must match its Codex source template"
fi
if ! grep -q 'plugin-health.ts' "$ROOT/packages/codex-plugin/bin/plugin-health.js"; then
...
expect_jq "packages/codex-plugin/.codex-plugin/plugin.json" \
  '.skills == "./skills/" and .commands == [] and .mcpServers == "./.mcp.json"' \
  "Codex plugin must expose package-local skills/MCP and disable best-effort command migration"
...
if find "$ROOT/packages/codex-plugin/skills" -mindepth 1 -maxdepth 1 -type d -name 'source-command-*' | grep -q .; then
