...
if [ -n "$mismatch" ]; then
  if [ "${1:-}" = "--fix" ]; then
    ...
    sed -i '' "s/\"version\": \"$dist_codex_plugin_version\"/\"version\": \"$synth_version\"/" "$DIST_CODEX_PLUGIN_JSON"
    if [ -n "$dist_codex_mcp_version" ]; then
      sed -i '' "s/const V='$dist_codex_mcp_version'/const V='$synth_version'/" "$DIST_CODEX_MCP_JSON"
    fi
