...
8. **Enter authenticated journeys through `cdp_login_prologue`.** It resolves only the exact `user-login` action and requires a fresh passing RunRecord. That result is a navigation helper, not PR proof. `LOGIN_PROLOGUE_BLOCKED` forbids unrelated mutations: do not use manual credentials, ad-hoc Maestro, navigation shortcuts, or store mutation afterward. Only when the result names the attach-only recovery for a fresh failed RunRecord with runner and bundle authority absent, run `device_snapshot(action="open", attachOnly=true)` on the already-bound app; stable Metro authority is required before and after. Rerun `cdp_login_prologue`, then repeat the attach-only open before native UI interaction. The latch remains set until the exact replay passes. Formal login proof is `cdp_lock_e2e_test` / `cdp_run_e2e_suite` on the exact candidate; helper and locked tests coexist without sharing that proof.
9. **Never infer UI control from health.** Session, device, Metro, Observe, runner, and bundle health are preconditions only; require the requested MCP mutation's own result to prove that it started and completed.
...
### Headless recovery — when `/mcp` is not available

A `claude -p` run cannot reconnect its own transport, so every remedy also names
...
`sameRootOwner` (`absent` / `live` / `stale` / `unprovable`), `ownerAppRoot` and a
truncated `ownerSession` naming the recorded holder (never its pid), any retained
`startupCleanupBlocked`, and the count of abandoned blocked contenders, and exits
...
unprovable, and exits non-zero when that owner holds this exact root — a live
owner of a different app root leaves nothing to clean up here, so that run exits
zero while `report` still names the holder. The abandoned claim-less contender
rows are reaped either way. There is no force-steal, by timeout or otherwise.
...
your own user, so a pid you cannot inspect is provably not it. An identity that
is unreadable for any other reason stays `unprovable` and keeps refusing.
...
- [ ] `rn_session({action: "status"})` does not report `state: blocked` — if it does, stop and follow § Session ownership recovery
