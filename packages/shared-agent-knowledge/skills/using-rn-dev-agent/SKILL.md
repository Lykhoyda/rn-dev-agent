...
Cursor, the Codex package root on Codex — the bundled runtime lives INSIDE the
installed package.)
...
8. **Enter authenticated journeys through `cdp_login_prologue`.** It resolves only the exact `user-login` action and requires a fresh passing RunRecord. That result is a navigation helper, not PR proof. A failed prologue fails like any other replay and latches nothing, but the journey still stops there: do not use manual credentials, ad-hoc Maestro, navigation shortcuts, or store mutation afterward. Formal login proof is `cdp_lock_e2e_test` / `cdp_run_e2e_suite` on the exact candidate; helper and locked tests coexist without sharing that proof.
