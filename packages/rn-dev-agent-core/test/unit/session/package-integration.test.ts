...
test('Metro descendant semantics bind arguments and Worker inputs', () => {
  ...
  try {
    ...
    writeFileSync(
      childEntry,
      "if (!process.send) { process.disconnect = () => process.exit(14); if (process.disconnect !== undefined) process.exit(15); } else { setInterval(() => {}, 1000); process.once('disconnect', () => process.exit(0)); process.once('message', () => { const disconnect = process.disconnect; const delegate = process._disconnect; const handleQueue = process._handleQueue; process.disconnect = () => process.exit(16); process._disconnect = () => process.exit(17); process._handleQueue = true; try { Object.defineProperty(process, '_disconnect', { value: () => process.exit(18) }); } catch {} try { Object.defineProperty(process, '_handleQueue', { value: true }); } catch {} if (process.disconnect !== disconnect || process._disconnect !== delegate || process._handleQueue !== handleQueue) process.exit(19); process.disconnect.call({ connected: true, _handleQueue: true }); if (process.connected) process.exit(20); }); }",
    );
    writeFileSync(
      workerEntry,
