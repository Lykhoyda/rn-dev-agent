...
test('Metro descendant semantics bind arguments and Worker inputs', () => {
  ...
  try {
    ...
    writeFileSync(
      childEntry,
      "if (process.send) { setInterval(() => {}, 1000); process.once('disconnect', () => process.exit(0)); process.once('message', () => { let replacementCalled = false; let replacementRejected = false; try { process.disconnect = () => { replacementCalled = true; }; } catch (error) { replacementRejected = error.code === 'RN_DEV_AGENT_UNSUPPORTED_DESCENDANT_EXECUTION'; } if (!replacementRejected) { process.disconnect.call({ connected: true, _handleQueue: true }); process.exit(replacementCalled && process.connected ? 14 : 15); } process.disconnect.call({ connected: true, _handleQueue: true }); if (process.connected) process.exit(16); }); }",
    );
    writeFileSync(
      workerEntry,
