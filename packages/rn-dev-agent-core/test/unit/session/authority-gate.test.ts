  ...
  const registry = {
    ...
    replaceBindingsDuringOperation: (operation, input) => {
      ...
      const committedOperation = {
        ...operation,
        authorityVersion: operation.authorityVersion + 1,
      };
...
test('failed authority probe after recovery cleans the advanced operation generation', async () => {
  ...
  registry.replaceBindingsDuringOperation = (operation, input) => {
    ...
    activeOperations.set(nextOperation.operationId, nextOperation);
    return nextOperation;
  };
  registry.endOperation = (operation) => {
    const active = activeOperations.get(operation.operationId);
    if (active?.authorityVersion !== operation.authorityVersion) {
      throw new SessionAuthorityError(
  ...
  let failAuthorityProbe = true;
  const gate = createAuthorityGate(runtime, {
    recoverRuntimeConnection: async () => true,
    refreshRuntimeBinding: async () => ({
      ...status.bindings.bundle,
      targetId: 'restored-target',
      connectionGeneration: status.bindings.bundle.connectionGeneration + 1,
    }),
    probe: async ({ axis }) => {
      if (failAuthorityProbe && axis === 'M') {
        throw new SessionAuthorityError(
          'METRO_AUTHORITY_MISMATCH',
          'the live Metro authority probe failed after recovery',
        );
      }
      return { axis, identity: `${axis}-identity` };
    },
  ...
  failAuthorityProbe = false;
  const following = await gate.wrap('cdp_console_log', async () => okResult({ entries: [] }))({});
  const envelope = JSON.parse(following.content[0].text);
  assert.equal(envelope.ok, true, following.content[0].text);
...
test('a failed login replay sets no latch, so mutating tools stay admissible', async () => {
  ...
  const prologue = await gate.wrap('cdp_login_prologue', async () =>
    failResult('cdp_run_action: user-login failed (SELECTOR_NOT_FOUND)', 'TESTID_NOT_FOUND'),
  )({});
  const prologueEnvelope = JSON.parse(prologue.content[0].text);
  assert.equal(prologueEnvelope.ok, false);
  assert.equal(prologueEnvelope.code, 'TESTID_NOT_FOUND');
  assert.equal(status.bindings.loginPrologue, undefined);

  for (const [tool, args] of [
    ['device_snapshot', { action: 'open' }],
    ['cdp_evaluate', { expression: 'continueJourney()' }],
    ['device_fill', { text: 'value' }],
    ['maestro_run', { yaml: '- tapOn: Login' }],
  ]) {
    let dispatched = false;
    const result = await gate.wrap(tool, async () => {
      dispatched = true;
      if (tool === 'device_snapshot') status.authorityVersion += 1;
      return okResult({ allowed: true });
    })(args);
    const envelope = JSON.parse(result.content[0].text);
    assert.equal(envelope.ok, true, `${tool}: ${result.content[0].text}`);
    assert.equal(dispatched, true, tool);
  }

  assert.equal(
    JSON.stringify(status.bindings).includes('LOGIN_PROLOGUE_BLOCKED'),
    false,
    'no login latch may be persisted into session bindings',
  );
});
