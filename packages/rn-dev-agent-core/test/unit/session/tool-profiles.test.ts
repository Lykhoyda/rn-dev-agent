...
// Every tool and argument-shaped special case must resolve to this exact profile.
...
test('Session resolves C+S; Target adds D+I; Runtime adds M+B with the live A probe; Automation adds R', () => {
  ...
  const nativeControl = authorityProfileFor('device_press');
  assert.deepEqual(nativeControl.groups, ['session', 'target', 'automation']);
  assert.deepEqual(nativeControl.axes, ['C', 'S', 'I', 'D', 'R']);
