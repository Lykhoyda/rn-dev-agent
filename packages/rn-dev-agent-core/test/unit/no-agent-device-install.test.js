...
// Resolve repo root: packages/rn-dev-agent-core/test/unit/ → ../../../../
const REPO_ROOT = resolve(__dirname, '../../../../');
const INSTALLER_PATH = resolve(REPO_ROOT, 'scripts/ensure-agent-device.sh');
const HOOK_PATH = resolve(REPO_ROOT, 'packages/claude-plugin/hooks/detect-rn-project.sh');
...
test('packages/claude-plugin/hooks/detect-rn-project.sh exists (sanity check)', () => {
  assert.ok(
    existsSync(HOOK_PATH),
    `packages/claude-plugin/hooks/detect-rn-project.sh not found at ${HOOK_PATH} — path resolution is wrong`,
  );
});

test('packages/claude-plugin/hooks/detect-rn-project.sh does not reference ensure-agent-device', () => {
...
test('packages/claude-plugin/hooks/detect-rn-project.sh still invokes ensure-android-ready.sh', () => {
  const src = readFileSync(HOOK_PATH, 'utf8');
  assert.match(
    src,
