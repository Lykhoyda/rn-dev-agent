...
test('preflight SETUP_ERROR for metro down / no device / app missing; null app tolerated', () => {
  assert.equal(
    preflight({ platform: 'ios', udid: 'u', metroReachable: false, appInstalled: true }).code,
    'SETUP_ERROR',
  );
  assert.equal(
    preflight({ platform: 'ios', udid: null, metroReachable: true, appInstalled: true }).code,
    'SETUP_ERROR',
  );
  assert.equal(
    preflight({ platform: 'ios', udid: 'u', metroReachable: true, appInstalled: false }).code,
    'SETUP_ERROR',
  );
  assert.equal(
    preflight({ platform: 'ios', udid: 'u', metroReachable: true, appInstalled: null }).ok,
    true,
  );
});
