...
async function rebindSessionRuntime(status) {
    ...
    const devClientUrl = (typeof prior?.devClientUrl === 'string' ? prior.devClientUrl : undefined) ??
        install.devClientUrl ??
...
trackedTool('cdp_connect', 'Connect only the exact app target on the authority-bound Metro and commit its signed initial-bundle handshake. Omitted port/platform/app values come from the session; conflicts refuse.', {
    ...
    platform: z
        .string()
        .optional()
        .describe('Filter target by platform (e.g. "ios", "android"). If already connected to a different platform, forces reconnection to the correct target.'),
    targetId: z
        .string()
        .optional()
    ...
    bundleId: z
        .string()
        .optional()
        .describe('App bundle id to match against target.description (e.g. "com.myapp.dev"). Filters out zombie Expo Go host pages when the real app target is present. B111/D635.'),
    force: z
        .boolean()
        .optional()
        .default(false)
...
trackedTool('cdp_component_tree', 'Get React component tree. Returns components with props, state, testIDs. Use filter to scope to a specific subtree — NEVER request full tree unless necessary (saves tokens). Detects RedBox and warns. Pass interactiveOnly=true for a compact "what can I act on here?" digest (only tappable/editable elements + their text, no props/state) — the cheapest way to perceive a novel screen for live interaction.', {
    filter: z
        .string()
        .optional()
        .describe('Case-insensitive substring match against component name, testID/nativeID, or accessibilityLabel (e.g. "CartBadge", "product-list", "Continue")'),
    depth: z.number().int().min(1).max(12).default(4).describe('Max depth (default 4, max 12)'),
    interactiveOnly: z
        .boolean()
        .optional()
        .describe('Return a compact salient digest: only actionable nodes (Pressable/Button/TextInput/Switch/Link + accessibilityRole controls) with {testID, role, text, label}, dropping props/hookStates/nesting. Ignores filter/depth. Use to cheaply see what is tappable on the current screen.'),
...
trackedTool('cdp_nav_graph', 'Navigation graph tool. PRIMARY: action="go" — navigates to any screen in ONE call (auto-scans if stale, plans path, executes via __NAV_REF__, verifies arrival, records outcome, returns heal advice on failure). Other actions for manual control: scan, read, navigate (plan only), record, staleness, playbook, heal.', {
    action: z
        ...
        .describe('go = navigate in one call (recommended). scan/read/navigate/record/staleness/playbook/heal for manual control'),
...
trackedTool('cdp_network_log', 'Get recent network requests. Shows method, URL, status, duration. On RN 0.83+ uses CDP Network domain. On older versions uses injected fetch/XHR hooks (auto-detected). Buffers are per-device, keyed by Metro port + target id — switching simulators no longer bleeds stale traffic. Pass `device: "all"` to merge across every device seen this session. Filters AND-combine: `filter` (URL substring), `method` (HTTP verb), `since` (ISO timestamp). When more entries match than `limit` allows, response includes `truncated:true` + `total_matches`.', {
    ...
    method: z
        ...
        .optional()
        .describe('Filter by HTTP method, case-insensitive (e.g. "POST" or ["POST","PUT"]). AND-combined with `filter`. Use to isolate mutations from follow-up GETs.'),
    since: z
        .string()
        .optional()
        .describe('ISO timestamp — drop entries with timestamp < since before applying limit. Use to pin a checkpoint before an action and ask for everything since.'),
    clear: z.boolean().default(false).describe('Clear network buffer instead of reading'),
    device: z
        .string()
        .optional()
        .describe('Scope: a specific device key OR the literal "all" for a chronologically-merged view across every device. Defaults to the active device.'),
...
trackedTool('cdp_network_body', 'Get the actual response body for a network request by its requestId. Use cdp_network_log first to find request IDs. In CDP mode (RN 0.83+) bodies are fetched on-demand; on RN < 0.83 hook mode a small recent-response cache is used. Pass `device` to look up requestId in a specific device buffer; defaults to the active device.', {
    ...
    device: z
        .string()
        .optional()
        .describe('Device key to scope the lookup ("all" to search every device buffer). Defaults to the active device.'),
...
trackedTool('cdp_wait_for_network', 'Block until a network request matching url_pattern (URL substring) and optional method completes (response received), or timeout_ms elapses. Two-phase: scans the existing buffer first (retroactive match), then polls every poll_interval_ms until deadline. Returns {matched:true, mutation, network_log_since} on success or {matched:false, timeout_ms, candidates_seen} (capped at 10) on timeout — never errors on timeout; agents should check `data.matched`. Use after triggering an action that fires a request to deterministically confirm it landed without buffer-churn races. Pin `since` to a timestamp captured BEFORE the trigger (Date.now() ISO) to also catch mutations that land in the MCP transport window. On RN < 0.83 (hook network mode) new-entry detection granularity is ~500ms — sub-500ms poll_interval_ms buys nothing there.', {
    url_pattern: z
        .string()
        .describe('URL substring to match (e.g. "/api/cart/add", "checkout"). Same matching semantics as cdp_network_log filter.'),
    method: z
        .union([z.string(), z.array(z.string())])
        .optional()
        .describe('HTTP method filter, case-insensitive (e.g. "POST" or ["POST","PUT"]). Omit to match any method.'),
    timeout_ms: z
        .number()
        .int()
        .min(100)
    ...
    since: z
        .string()
        .optional()
        .describe('ISO timestamp checkpoint — ignore entries older than this. Defaults to the moment the tool is called. Capture `new Date().toISOString()` before the trigger action to avoid missing the mutation in the transport window.'),
...
trackedTool('cdp_dispatch', 'Dispatch a Redux action and optionally read state afterward — all in a single synchronous JS execution. Use for atomic dispatch+verify operations (e.g. dispatch "tasks/softDelete" then read "tasks.pendingDelete"). NOTE: Best used for state verification, not UI interaction testing — React components may not re-render immediately after CDP-dispatched actions. For UI testing, use device_press/device_find to trigger the action through the UI instead.', {
    ...
    payload: z
        .any()
        .optional()
        .describe('Action payload. WARNING: JSON-RPC between LLM and MCP does not preserve the distinction between string "42" and number 42 — the LLM\'s JSON encoder may serialize either way. For type-critical payloads (e.g. a string that happens to be numeric), use payloadJson instead.'),
    payloadJson: z
        .string()
        .optional()
        .describe('Stringified JSON payload with guaranteed type preservation. Takes precedence over `payload` when provided. Example: payloadJson=\'"42"\' dispatches the STRING "42"; payloadJson=\'42\' dispatches the NUMBER 42; payloadJson=\'{"id":"42","qty":5}\' dispatches an object.'),
    readPath: z
        .string()
        .optional()
...
trackedTool('cdp_interact', 'Interact with React components by testID (preferred) or accessibilityLabel — press buttons, long-press, type text, scroll, or set a React Hook Form field value directly. Calls JS handlers directly (not native touch). testID matches strictly; accessibilityLabel matches in tiers (exact → trim/case-insensitive → substring) and returns an ambiguity error when >1 component matches. Prefer testID for unambiguous targeting. For native gestures (swipe, drag), use device_swipe/device_press instead. setFieldValue (GH #126 Gap A): explicit fallback when typeText fails because the field routes through a Controller — pass name + value, walks UP to the nearest FormProvider and calls its setValue. Use only when typeText returns "no handler". Portal-root coverage (GH #126 Gap B): if your app uses react-native-actions-sheet, @gorhom/bottom-sheet, or any Modal-based portal whose fiber root is not in React DevTools\' getFiberRoots() registry, set `globalThis.__RN_AGENT_EXTRA_ROOTS__ = () => [sheetRef.current, ...]` in your __DEV__ block — testID resolution will then reach inside those subtrees. See CLAUDE.md template for the canonical snippet.', {
    ...
    testID: z
        .string()
        .optional()
        .describe("testID prop of the target component (strict match — preferred). For setFieldValue, this is the testID anchor inside the form's subtree from which to walk up."),
    ...
    shouldValidate: z
        .boolean()
        .optional()
        .describe("For setFieldValue: pass-through to setValue's options.shouldValidate (default true). Set false to suppress synchronous validation."),
    shouldDirty: z
        .boolean()
        .optional()
...
trackedTool('device_screenshot', 'Capture the exact authority-bound device screen with no cross-device retry. Raw control requires exact install/device/runner authority but not a managed Metro target; meta.originAuthority explicitly reports proven or not-proven, and not-proven screenshots are never strict source evidence. Returns the file path; iOS failures preserve sanitized backend argv, exit/signal/timeout, stderr, output format/path, and a shortened receipt-bound device identity.', {
    ...
    maxWidth: z
        ...
        .min(0)
        .optional()
        .describe('Downscale image so width does not exceed this many pixels. 0 disables resize. Default 800 (saves ~46% on iPhone 15/17 Pro screenshots without losing label readability).'),
    quality: z
        .number()
        .int()
        .min(1)
...
trackedTool('device_snapshot', 'Manage exact device sessions and capture UI snapshots even when a Dev Client remains at its native picker. Raw control requires exact install/device/runner authority but not a managed Metro target; meta.originAuthority explicitly reports proven or not-proven, and not-proven snapshots are never strict source evidence. action=open starts a session (required before other device_ tools), waits for Android app accessibility, and reports readiness.reactNativeUi=ready only when a matching live CDP helper confirms the RN fiber boundary; otherwise it warns that RN readiness is unverified. Pass deviceId to select an exact iOS simulator UDID or Android adb serial when devices run in parallel. action=snapshot returns the accessibility tree with @ref identifiers for device_press/device_fill. action=close ends the session. Use attachOnly=true on action=open to skip launching the app when it is already running (avoids relaunch-induced bundle races); liveness is checked only on the resolved exact device and refuses when that identity is unavailable.', {
    action: z
        ...
        .default('snapshot')
        .describe('open: start session for an app. snapshot: capture UI tree with element refs. close: end session.'),
    appId: z
        .string()
        .optional()
...
trackedTool('device_swipe', 'Swipe on the device screen. Use direction for simple scrolling, or x1/y1/x2/y2 for precise coordinate-based swipes (drag-to-reorder, bottom sheets). Pass exact: true for a precise unclamped gesture duration via the in-tree runner — needed for momentum-sensitive UIs like UIDatePicker wheels where a normalized/clamped duration causes overshoot. Requires an open session.', {
    ...
    durationMs: z
        ...
        .max(10000)
        .optional()
        .describe('Swipe duration in ms (slower = more precise, default ~300). Note: the non-exact path may normalize/clamp very short durations — use exact: true for an unclamped duration.'),
    count: z
        .number()
        .int()
        .min(1)
    ...
    pattern: z
        ...
        .optional()
        .describe('Repeat pattern: one-way (reset to start) or ping-pong (reverse direction). Incompatible with exact: true.'),
    exact: z
        .boolean()
        .optional()
        .describe('B123: REQUIRE fast-runner (no daemon fallback). Preserves user-supplied durationMs verbatim — needed for slow precise swipes on UIDatePicker wheels and similar momentum-sensitive UIs. Fails with EXACT_REQUIRES_FAST_RUNNER if fast-runner unavailable instead of silently degrading.'),
...
trackedTool('device_reset_state', 'Reset permissions/storage and relaunch the authority-bound app on its exact claimed device, then reconnect and re-prove the session target.', {
    ...
    storageKeys: z
        .array(z.string())
        .optional()
        .describe('MMKV keys to delete before terminate (so the app reads cleared values on next launch). Skipped if CDP is not connected.'),
    mmkvInstanceId: z
        .string()
        .optional()
    ...
    waitForReady: z
        .boolean()
        .optional()
        .describe('After relaunch, wait for CDP reconnect + helpers injection. Default true. Set false to return immediately and let the caller poll.'),
    waitForNavReady: z
        .boolean()
        .optional()
...
trackedTool('device_accept_system_dialog', 'Tap an OS-level accept button on the exact session device. iOS prefers the capability-bound native runner so SpringBoard-owned dialogs are reachable; DIALOG_BUTTON_NOT_FOUND returns availableButtons for an exact-label retry.', {
    label: z
        .string()
        .optional()
        .describe('Specific button label to tap. Omit to try common defaults (Allow, OK, Open, Continue, Yes, Accept).'),
    platform: z
        .enum(['ios', 'android'])
        .optional()
        .describe('Authority-bound platform; conflicting values are refused'),
...
trackedTool('device_dismiss_system_dialog', 'Tap an OS-level dismiss button through the capability-bound runner on the exact session device.', {
    label: z
        .string()
        .optional()
        .describe('Specific button label to tap. Omit to try common defaults (Cancel, Don\u2019t Allow, Deny, No, Not Now).'),
    platform: z
        .enum(['ios', 'android'])
        .optional()
        .describe('Authority-bound platform; conflicting values are refused'),
...
trackedTool('device_record', 'Record the exact authority-bound device for proof capture. Start validates that the claimed device is currently available and always forwards its literal identifier.', {
    ...
    outputPath: z
        .string()
        .optional()
        .describe('(start only) Absolute output path. Defaults to /tmp/rn-dev-agent-proof-<platform>-<timestamp>.mp4.'),
    deviceId: z
        .string()
        .optional()
        .describe('Authority-bound exact device identifier; normally injected by the session'),
    ...
    gifPath: z
        .string()
        .optional()
        .describe('(stop only) Override GIF output path. Defaults to the recording path with .gif extension.'),
...
trackedTool('device_pick_value', 'Select a value in a UIPickerView / Android picker wheel by tapping the target row. Works for any picker that exposes row labels via accessibility. If pickerTestId is provided, taps the picker open first. Known limitation: only works when the target value is already visible in the wheel window (scroll-to-visible is not yet implemented).', {
    ...
    pickerTestId: z
        .string()
        .optional()
        .describe('Optional testID of the picker itself — tapped first to ensure the picker is open.'),
    platform: z
        .enum(['ios', 'android'])
        .optional()
...
trackedTool('maestro_run', 'Execute a Maestro flow via maestro-runner. Pass flowPath for an existing .yaml file, or inlineYaml for ephemeral flows. Uses UIAutomator2 on Android and XCTest on iOS. A matching active device session is forwarded as an exact --device/--udid target; maestro-runner success is rejected unless its direct device/WDA evidence matches. Does NOT require CDP — works even when app is crashed or on native screens.', {
    ...
    params: z
        ...
        .optional()
        .describe('GH #116: parameter bindings forwarded as -e KEY=VALUE for ${KEY} placeholders in the flow. Keys must match /^[A-Z_][A-Z0-9_]*$/ (validated in the handler).'),
...
trackedTool('maestro_generate', 'Generate a persistent Maestro YAML flow file from structured steps. Writes to .rn-agent/actions/<name>.yaml in the project root. Use after live verification to create reusable actions.', {
    ...
    outputDir: z
        .string()
        .optional()
        .describe('Output directory (default: <project>/.rn-agent/actions/). Pass an explicit path for non-default targets.'),
...
trackedTool('cdp_record_test_generate', 'Render the stored recording as replayable test code. Formats: maestro (YAML, primary), detox (JS). Appium returns NOT_IMPLEMENTED — file an issue if you need it. Requires a recording in memory (call start/stop or load first). Pass id/intent/tags/mutates/status to emit the M7 metadata header (D1203) into the YAML so the result is a first-class reusable action.', {
    ...
    id: z
        .string()
        .optional()
        .describe('M7 action id (stable slug). When set, emitted as `# id: <slug>` header line. Default: filename without `.yaml`.'),
    intent: z
        .string()
        .optional()
...
trackedTool('cross_platform_verify', 'Compare UI elements across iOS and Android. Reads cached accessibility snapshots from both platforms (populated by device_snapshot) and checks which elements are present on each. Workflow: test on iOS → device_snapshot → switch to Android → device_snapshot → cross_platform_verify. Supports auto-discovery of testIDs from source via scanDir. Returns a per-element comparison table with PASS/FAIL verdict.', {
    elements: z
        .array(z.string())
        .optional()
        .describe('List of testIDs or labels to check on both platforms. Optional if scanDir is provided.'),
    scanDir: z
        .string()
        .optional()
        .describe('Directory to scan for testID="..." props in .tsx/.jsx/.ts/.js files. Auto-discovers elements. Merges with elements[] if both provided.'),
    matchBy: z
...
trackedTool('expect_redux', 'Assert against Redux/Zustand store state at a path. Returns ok when the assertion matches; failResult with code=ASSERTION_FAILED when it does not. Operators (compose with AND): equals (deep), exists (default if no other op), notExists, length (array/string), contains (array), gt/lt/gte/lte (numbers). Pass timeoutMs to retry until match — useful when the store updates asynchronously after a tap. Differentiated capability over Maestro: Maestro asserts pixels; this asserts internal state.', {
    ...
    storeType: z
        .string()
        .optional()
        .describe('Restrict to a specific store ("redux" | "zustand" | a Zustand store name). Default: auto-detect.'),
    equals: z.unknown().optional().describe('Deep-equal against this value.'),
    exists: z
        .boolean()
        .optional()
        .describe('When true, value must be defined and non-null. When false, value must be undefined or null. Implicit default if no other operator is supplied.'),
...
trackedTool('expect_route', "Assert against the navigation state — current route name, current route params, or a route's presence in the stack. Returns ok when the assertion matches; failResult with code=ASSERTION_FAILED otherwise. Differentiated capability over Maestro: Maestro doesn't know what route you're on, only what's rendered. Pass timeoutMs to retry through navigation animations.", {
    ...
    inStack: z
        .string()
        .optional()
        .describe('Asserts a route with this name exists somewhere in the stack (not necessarily current).'),
    timeoutMs: z
        .number()
        .int()
        .min(0)
        .optional()
        .describe('Polling timeout in ms (default 0). Use 1000-2000 to wait through navigation animations.'),
...
trackedTool('cdp_record_test_save_as_action', 'Promote the in-memory recording (started via cdp_record_test_start) into a first-class L3 reusable action. Writes Maestro YAML with full M7 metadata header (id, intent, tags, mutates, status, produces) to <project>/.rn-agent/actions/<id>.yaml and initialises the sidecar runtime state. Status defaults to "experimental" — first clean /run-action replay auto-promotes to "active". Refuses if the id already exists unless overwrite=true. Distinct from cdp_record_test_save (which writes JSON to .rn-agent/recordings/) — that is for raw event archival; this is for shipping the recording as a replayable action. The optional `produces` field (D1209) records state postconditions — what state the action establishes when it runs cleanly — so downstream tasks can use it as a deterministic prologue.', {
    ...
    tags: z
        .array(z.string())
        .optional()
        .describe('Lower-case kebab-case keywords for filtering (e.g. ["tasks", "create", "regression"]).'),
    mutates: z
        .boolean()
        .optional()
        .describe('Does this flow leave persistent residue (created rows, toggled settings)? Required for /run-action safety pre-flight to know whether to confirm before replay.'),
    status: z
        .enum(['experimental', 'active', 'deprecated'])
        .optional()
        .describe('M7 lifecycle status. Default: experimental (auto-promotes on first clean replay).'),
    bundleId: z
        .string()
        .optional()
        .describe('App bundle ID for the Maestro appId header. Strongly recommended — /run-action uses it to refuse cross-app replays.'),
    projectRoot: z.string().optional().describe('Override project root (default: process.cwd()).'),
    overwrite: z
        .boolean()
        .optional()
        .describe('If an action with this id already exists, replace it. Default false (refuse with hint).'),
    testName: z
        .string()
        .optional()
        .describe('Optional one-line description shown as a comment above the M7 header. Falls back to intent.'),
    produces: z
        .record(z.union([z.string(), z.number(), z.boolean()]))
        .optional()
        .describe('D1209 — state postconditions this action establishes when it runs cleanly. Flat map of primitive values for hybrid composition (e.g. { authenticated: true, route: "home" }). Optional. Values containing commas or newlines are not supported; use multiple keys instead.'),
...
trackedTool('cdp_repair_action', 'Repair a learned action using a fresh snapshot from the exact authority-bound device and capability-bound native runner.', {
    ...
    threshold: z
        ...
        .max(1)
        .optional()
        .describe('Fuzzy-match similarity threshold (0..1). Default 0.6. Lower if the screen has many similar testIDs and Levenshtein on the original is too strict.'),
    dryRun: z
        .boolean()
        .optional()
        .describe("Don't write changes — return the diff that WOULD be applied. Useful for previewing repairs before committing."),
    agentReasoning: z
        .string()
        .optional()
...
trackedTool('cdp_run_action', "Replay a learned action by id with end-to-end auto-repair. Loads the action from .rn-agent/actions/<actionId>.yaml, forwards the matching active session's exact device ID to Maestro, rejects mismatched direct runner/WDA evidence, and on a SELECTOR_NOT_FOUND failure automatically invokes cdp_repair_action and retries once. Appends a RunRecord to the sidecar with full auto-repair telemetry (passed/failed/refused/skipped + diff); its Maestro deviceId comes from direct runner evidence, never requested metadata. The repair attempt counts toward cdp_repair_action's 24h budget. Pass autoRepair=false to opt out of auto-repair (returns the raw maestro_run failure verbatim). forceReload defaults true: any human edit to the YAML since the agent's last write is acknowledged as the new baseline so downstream repair does not abort with STALE_TARGET (the right default for active composition). Pass forceReload=false for the strict \"respect offline human edits\" behavior: a successful replay still appends its RunRecord to the sidecar when only the tracked YAML mtime baseline is stale, while YAML-mutating promotion and repair stay refused. proofReplay=true is reserved for proof_capture rehearsal and requires autoRepair=false plus forceReload=false; it executes without RunRecord, promotion, YAML, sidecar, or DB persistence. The orchestrated home for the L3 self-healing loop — prefer this over invoking maestro_run + cdp_repair_action manually for any flow you intend to re-run on schedule. blindProbeMode provides per-call control of the proactive CDP/JS compatibility path: inherit (default) honors RN_BLIND_PROBE, allow explicitly enables it for this call, and forbid forces maestro-first for this call.", {
    ...
    autoRepair: z
        .boolean()
        .optional()
        .describe('Auto-repair on SELECTOR_NOT_FOUND failures. Default true. Pass false to disable (e.g. when investigating a failure manually).'),
    timeoutMs: z
        .number()
        .optional()
...
setObserveE2eDeps({
    ...
    runAction: async (actionId, params) => {
        ...
        if (required.length > 0) {
            ...
            if (!resolved.ok)
                return { ok: false, missingParams: resolved.missing };
