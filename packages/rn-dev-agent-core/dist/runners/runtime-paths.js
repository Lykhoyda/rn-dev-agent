...
export function candidateNativeRunnerDirs(runnerName, baseDir = import.meta.dirname) {
...
export function resolveNativeRunnerDir(runnerName, baseDir = import.meta.dirname) {
    const candidates = candidateNativeRunnerDirs(runnerName, baseDir);
    return candidates.find(isDirectory) ?? candidates[0];
}
export function candidateRunnerManifestFiles(baseDir = import.meta.dirname) {
    const repoRoot = process.env.RN_DEV_AGENT_ROOT;
    const codexPluginRoot = process.env.RN_DEV_AGENT_CODEX_PLUGIN_ROOT;
    const claudePluginRoot = process.env.CLAUDE_PLUGIN_ROOT;
    return compactUnique([
...
export function candidatePluginManifestFiles(baseDir = import.meta.dirname) {
    const codexPluginRoot = process.env.RN_DEV_AGENT_CODEX_PLUGIN_ROOT;
    const claudePluginRoot = process.env.CLAUDE_PLUGIN_ROOT;
    return compactUnique([
