...
async function executeAndroidPlatformCommand(deviceId, args, signal, execute = async (commandArgs, options) => execFileAsync('adb', commandArgs, options)) {
...
export async function removeAndroidForwardWithOutcome(deviceId, hostPort, signal, execute) {
    return (await executeAndroidPlatformCommand(deviceId, buildAdbForwardRemoveArgs(deviceId, hostPort), signal, execute)).outcome;
}
...
export function shouldReapAndroidRunnerBeforeStart(state, deviceId, isAvailable) {
...
export function androidHealthMatchesAuthority(info, expected) {
    return (info.instanceId === expected.instanceId &&
...
function exactAndroidForwardLine(expected, forwards) {
    return forwards
        ...
        if (extra !== undefined || deviceId !== expected.deviceId)
            return false;
        if (expected.hostPort !== undefined && local !== `tcp:${expected.hostPort}`)
            return false;
        return expected.devicePort === undefined || remote === `tcp:${expected.devicePort}`;
    });
}
export function classifyAndroidRunnerCleanupResources(expected, forwards, processes) {
    ...
    for (const line of processLines.slice(activeBlockStart + 1)) {
        ...
        if (line.search(/\S/) <= activeBlockIndent)
            break;
        if (/ActiveInstrumentation\{/.test(line) ||
...
export async function verifyAndroidRunnerCleanupResources(expected, forwards, commandEvidence = {}, signal, execute) {
    const processes = await executeAndroidPlatformCommand(expected.deviceId, ['-s', expected.deviceId, 'shell', 'dumpsys', 'activity', 'processes'], signal, execute);
    const evidence = {
        ...commandEvidence,
        instrumentationCleanup: processes.outcome,
    };
    if (processes.outcome.exitCode !== 0 ||
        processes.outcome.signal !== null ||
        processes.outcome.timedOut) {
        throw new AndroidRunnerCleanupUnconfirmedError('instrumentation', `argv=${JSON.stringify(processes.outcome.argv)} ` +
            ...
            `stderrTail=${JSON.stringify(processes.outcome.stderrTail)}`, expected.deviceId, evidence);
    }
...
export async function reapMismatchedAndroidRunner(state, release, verify, signal, verification = {}) {
    ...
    if (state?.hostPort !== undefined &&
        ...
        const listenerNotFound = remove.outcome.exitCode !== null &&
            ...
            (remove.outcome.stderrTail === expectedMissingListener ||
                remove.outcome.stderrTail === `${expectedMissingListener}\n` ||
                remove.outcome.stderrTail === `${expectedMissingListener}\r\n`);
        const idempotentMissingForward = !commandFailed(before.outcome) &&
        ...
        if (commandFailed(before.outcome) ||
            ...
            const failed = commandFailed(remove.outcome)
                ...
                : commandFailed(after.outcome)
                    ? after.outcome
                    : before.outcome;
            throw new AndroidRunnerCleanupUnconfirmedError('forward-release', `argv=${JSON.stringify(failed.argv)} exitCode=${String(failed.exitCode)} ` +
                `signal=${String(failed.signal)} timedOut=${String(failed.timedOut)} ` +
                `stderrTail=${JSON.stringify(failed.stderrTail)}`, deviceId, commandEvidence);
        }
        const retained = exactAndroidForwardLine(expected, after.stdout);
        if (retained) {
            throw new AndroidRunnerCleanupUnconfirmedError('forward', sanitizedCommandValue(retained, deviceId), deviceId, commandEvidence);
        }
    }
    const commandEvidence = {
