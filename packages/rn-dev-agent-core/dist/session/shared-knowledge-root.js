import { cpSync, lstatSync, readdirSync, readlinkSync, renameSync, rmSync, statSync, } from 'node:fs';
import { join, resolve } from 'node:path';
function assertTreeHasNoLinks(root) {
    for (const entry of readdirSync(root, { withFileTypes: true })) {
        const path = join(root, entry.name);
        if (lstatSync(path).isSymbolicLink()) {
            throw new Error('SHARED_KNOWLEDGE_ROOT_UNSAFE: nested symlinks cannot be materialized safely');
        }
        if (entry.isDirectory())
            assertTreeHasNoLinks(path);
    }
}
export function ensureSharedKnowledgeRoot(appRoot) {
    ...
    try {
        ...
        try {
            ...
            throw new Error('SHARED_KNOWLEDGE_ROOT_UNSAFE: materialization failed and symlink restoration failed', { cause: error });
        }
        throw error;
    }
