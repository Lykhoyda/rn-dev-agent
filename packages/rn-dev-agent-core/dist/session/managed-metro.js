...
function resolveWindowsPackageCommand(appRoot, packageName, commandName, args, dependencies) {
    ...
    const manifest = JSON.parse(dependencies.readText(join(packageRoot, 'package.json')));
...
function verifyManagedMetroRuntimeAdmission(path, capability, expected) {
    try {
        ...
        if (!observedDescendantAuthority ||
...
function readManagedMetroLauncherDiagnostic(path) {
    try {
        ...
        if (Buffer.byteLength(source) > 4_096)
            return null;
...
function managedMetroStartupError(input) {
    ...
    if (violation && /^[A-Z][A-Z0-9_]+:/.test(violation)) {
        return new Error(`${sanitizeManagedMetroStartupDetail(violation, redactions)}${details.length > 0 ? `; ${details.join('; ')}` : ''}`.slice(0, 4_096));
    }
    ...
    return new Error(`METRO_START_UNAVAILABLE: allocated Metro did not become authoritative${details.length > 0 ? ` (${details.join('; ')})` : ''}`.slice(0, 4_096));
}
...
export async function startManagedMetro(input, dependencies = {}) {
    ...
    const runtimePolicyPath = join(input.appRoot, '.rn-agent', 'integration', 'metro-runtime-policy.json');
...
export function inspectManagedMetroCleanupEvidence(binding, dependencies = {}) {
    const probeBirth = dependencies.probeBirth ?? probeProcessBirth;
    const probeListener = dependencies.probeListener ?? probeManagedMetroListener;
    ...
    const evidenceSocket = managed
        ...
        : 'not-applicable';
    const complete = launcher !== 'present' &&
        launcher !== 'unknown' &&
        listener === 'absent' &&
        port.status === 'absent' &&
...
export async function stopManagedMetro(binding, input, dependencies = {}) {
    ...
    const observedBuffer = Buffer.from(binding.managementProof, 'hex');
    const expectedProofs = binding.runtimeEvidenceAuthority === undefined
...
export async function stopManagedMetroWithEvidence(binding, input, dependencies = {}) {
    const proofAuthenticated = binding !== null &&
        binding !== undefined &&
        verifyManagedMetroManagementProof(binding, input);
