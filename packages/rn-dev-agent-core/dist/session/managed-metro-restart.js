 ...
 */
export function resolveManagedMetroRestartGeneration(input) {
    if (!input.install) {
        if (input.pendingBuild) {
            const generation = input.pendingBuild.buildGeneration;
            if (input.pendingBuild.platform !== input.device.platform ||
        ...
        return (Math.max(0, ...input.priorGenerations.map((generation) => Number.isSafeInteger(generation) ? Number(generation) : 0)) + 1);
    }
