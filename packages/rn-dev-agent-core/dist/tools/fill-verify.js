...
export function classifyFiberVerification(input) {
...
export function combineVerificationOracles(fiber, native, nativeStable) {
    ...
    const observedMismatch = !verified &&
        !targetCompromised &&
        !anyExact &&
...
export function decideNativeRetype(verification, attemptsSoFar, maxAttempts) {
...
export async function probeInputState(deps, testID) {
 ...
 */
export async function settleRead(deps, testID, text, valueBefore) {
    ...
    for (let i = 0; i < READ_SETTLE_TRIES; i++) {
        ...
        if (i < READ_SETTLE_TRIES - 1)
            await sleep(READ_SETTLE_DELAY_MS);
    }
 ...
 */
export async function finalFiberVerify(deps, testID, text) {
    const sleep = deps.sleep ?? ((ms) => new Promise((r) => setTimeout(r, ms)));
    let previous = null;
    let last = null;
    for (let i = 0; i < FINAL_VERIFY_TRIES; i++) {
        ...
        if (i < FINAL_VERIFY_TRIES - 1)
            await sleep(FINAL_VERIFY_DELAY_MS);
    }
