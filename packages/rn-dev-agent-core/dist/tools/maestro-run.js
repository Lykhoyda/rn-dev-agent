...
import { completeManagedRunnerParkAuthority, claimManagedNativeOriginAuthority, completeManagedNativeOriginAuthority, hasManagedNativeOriginAuthority, hasManagedInstallReissueAuthority, reissueManagedInstallAuthority, relaunchManagedNativeOriginApp, reproveManagedNativeOrigin, } from '../session/authority-gate.js';
import { SessionAuthorityError } from '../session/registry.js';
import { planIosProofDomains, loginPostconditionId, nativeSelectorsForCommands, } from '../domain/ios-proof-router.js';
import { runCdpReplayCommands } from './cdp-replay-dispatch.js';
...
export async function executeMaestroAuthorityStages(commands, executeStage, claimOrigin, completeOrigin, relaunchManagedApp, reproveManagedOrigin, options = {}) {
...
function isLoginMetadata(metadata) {
    if (!metadata)
        return false;
    return (/(^|[-_.])login($|[-_.])/.test(metadata.id) ||
        metadata.tags?.some((tag) => tag === 'login' || tag === 'auth') === true);
}
async function defaultProbeAndroidApiLevel(deviceId) {
    try {
        const { stdout } = await defaultExecFile('adb', ['-s', deviceId, 'shell', 'getprop', 'ro.build.version.sdk'], { timeout: 5000, encoding: 'utf8', maxBuffer: 1024 * 1024 });
...
export function createMaestroRunHandler(deps = {}) {
    ...
    return async (args) => {
        ...
        const semanticActionMeta = capturedAction?.metadata ??
            args.actionMetadata ??
            (args.flowPath
                ? parseM7Header(rawYaml, basename(args.flowPath).replace(/\.ya?ml$/i, ''))
                : null);
        const iosProofPlan = platform === 'ios' && replayFactory
        ...
        if (iosProofPlan?.ok &&
            ...
            const deadlineTimer = setTimeout(() => controller.abort(new Error('iOS partitioned replay deadline exceeded')), timeout);
            ...
            try {
                for (const segment of iosProofPlan.segments) {
                    ...
                    if (segment.domain === 'xctest-native') {
                        ...
                        if (env.ok !== true || env.data?.passed !== true) {
                            ...
                            const proofDomain = uniqueProofDomains.length === 1
                ...
                if (failure instanceof ReactReplayFailure) {
                    ...
                    const failedStepIndex = replay.failedStepIndex === undefined
                        ? undefined
                ...
                if (failure instanceof SessionAuthorityError)
                    throw failure;
                return failResult(failure instanceof Error ? failure.message : String(failure), controller.signal.aborted ? 'RUNNER_TIMEOUT' : undefined, { proofDomains });
            }
            finally {
        ...
        try {
            ...
            const selectorLessAssertionFailure = nativeFailure.kind === 'UNKNOWN' &&
                terminal.exitClass === 'step-failure' &&
                terminal.failedStep?.split(/\s+/, 1)[0] === 'assertVisible';
            const failedNativeSelector = nativeFailure.kind === 'SELECTOR_NOT_FOUND'
            ...
            const fastRunnerSawFailedSelector = failedNativeSelector !== null &&
                nativeVisionEvidence?.visibleSelectors.some((selector) => selector.value === failedNativeSelector) === true;
