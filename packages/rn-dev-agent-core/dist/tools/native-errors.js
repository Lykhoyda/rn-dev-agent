...
async function defaultRunAndroid(sinceSeconds, deviceId) {
    ...
    const { stdout } = await execFile('adb', ['-s', deviceId, 'logcat', '-d', '-v', 'time', '-t', `${sinceSeconds * 100}`, '*:E'], { timeout: 10_000, encoding: 'utf8', maxBuffer: 10 * 1024 * 1024 });
    return stdout;
}
export async function readNativeErrors(opts = {}) {
    ...
    try {
        if (platform === 'android') {
            const out = await (opts.runAndroid ?? (() => defaultRunAndroid(sinceSeconds, String(opts.deviceId))))();
            return {
                ok: true,
