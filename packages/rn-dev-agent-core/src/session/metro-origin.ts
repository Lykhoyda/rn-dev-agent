...
export interface ForeignMetroOriginQuery {
...
export interface ForeignMetroOriginEvidence {
  port: number;
  targetDeviceName: string;
}

export type ForeignMetroOriginScanner = (
  query: ForeignMetroOriginQuery,
) => Promise<ForeignMetroOriginEvidence | null>;

export interface ForeignMetroOriginDependencies {
  listSiblingMetroPorts(expectedMetroPort: number): Promise<number[]>;
  fetchPortTargets(port: number): Promise<HermesTarget[]>;
  filterForExactDevice<T extends { deviceName?: string | null }>(input: {
    platform: 'ios' | 'android';
    deviceId: string;
    targets: readonly T[];
  }): Promise<T[]>;
...
function targetServedByPort(target: HermesTarget, port: number): boolean {
  try {
 ...
 */
export async function findForeignMetroOrigin(
  query: ForeignMetroOriginQuery,
  dependencies: ForeignMetroOriginDependencies,
): Promise<ForeignMetroOriginEvidence | null> {
  let siblingPorts: number[];
  try {
    siblingPorts = (await dependencies.listSiblingMetroPorts(query.expectedMetroPort)).filter(
      (port) => port !== query.expectedMetroPort,
    );
  } catch {
    return null;
  }
  if (siblingPorts.length === 0) return null;
  const candidates: Array<{ deviceName?: string; port: number }> = [];
  await Promise.all(
    siblingPorts.map(async (port) => {
      try {
        ...
        const targets = filterValidTargets(await dependencies.fetchPortTargets(port)).filter(
          (target) =>
            targetMatchesBundleId(target, query.appId) &&
            targetServedByPort(target, port) &&
            (inferPlatformFromDeviceName(target.deviceName) ?? query.platform) === query.platform,
        );
        for (const target of targets) candidates.push({ deviceName: target.deviceName, port });
      } catch {
  ...
  let matches: Array<{ deviceName?: string; port: number }>;
  try {
...
export interface MemoizedForeignMetroOriginScanner extends ForeignMetroOriginScanner {
  invalidate(query?: ForeignMetroOriginQuery): void;
}

function scanCacheKey(query: ForeignMetroOriginQuery): string {
...
export function memoizeForeignMetroOriginScanner(
  scan: ForeignMetroOriginScanner,
  options: {
    now?: () => number;
    unprovableTtlMs?: number;
    evidenceTtlMs?: number;
    capacity?: number;
  } = {},
): MemoizedForeignMetroOriginScanner {
...
export function createForeignMetroOriginScanner(
  authority: TargetDeviceAuthorityDependencies,
  overrides: Partial<ForeignMetroOriginDependencies> = {},
): MemoizedForeignMetroOriginScanner {
  const dependencies: ForeignMetroOriginDependencies = {
    listSiblingMetroPorts:
      overrides.listSiblingMetroPorts ??
      ((expectedMetroPort) =>
        discoverAllMetroPorts(
          [...new Set(resolveDefaultPorts())].filter((port) => port !== expectedMetroPort),
          DISCOVERY_TIMEOUT_MS,
        )),
    fetchPortTargets:
      overrides.fetchPortTargets ?? ((port) => fetchTargets(port, DISCOVERY_TIMEOUT_MS)),
    filterForExactDevice:
      overrides.filterForExactDevice ?? ((input) => filterTargetsForExactDevice(input, authority)),
  };
...
export function provenMetroOriginMismatch(
  expectedMetroPort: number,
  context: { platform: 'ios' | 'android'; deviceId: string; appId: string },
  evidence: ForeignMetroOriginEvidence,
): SessionAuthorityError {
  const error = new SessionAuthorityError(
    'METRO_ORIGIN_MISMATCH',
    `the bound ${context.platform} device ${context.deviceId} is running ${context.appId} served by Metro :${evidence.port}, not this session's Metro :${expectedMetroPort}`,
    undefined,
    {
      expected: `Metro :${expectedMetroPort}`,
      observed: `Metro :${evidence.port} (target on ${evidence.targetDeviceName})`,
      nextAction:
        'Re-point the dev client to this session\'s Metro with rn_session action "pin_dev_client", then retry.',
    },
  );
  error.attachMeta({
