...
export interface ExpoAndroidDeviceBinding {
  /** Durable authority. This value is always the exact adb serial. */
  deviceId: string;
  /** Ephemeral Expo CLI compatibility value. Never persist this as authority. */
  displayName: string;
}
...
export function expoAndroidDeviceIdentityError(message: string): Error {
  const error = new Error(`${ERROR_CODE}: ${message}`) as Error & { code: string };
...
export function parseAdbDevices(output: string): AdbDevice[] {
  ...
  for (const rawLine of output.split(/\r?\n/)) {
    ...
    if (!serial || !state || !ANDROID_SERIAL_RE.test(serial)) continue;
    if (seen.has(serial)) {
      throw expoAndroidDeviceIdentityError(
        'adb returned duplicate records for one serial; disconnect stale transports and retry',
      );
    }
    seen.add(serial);
    devices.push({
...
function emulatorDisplayName(output: string): string {
  ...
  if (!name || name === 'OK' || name.startsWith('KO:')) {
    throw expoAndroidDeviceIdentityError(
      'the connected emulator AVD name is unavailable; restart that exact emulator and retry',
    );
  }
  return name;
}

function isAuthorized(device: AdbDevice): boolean {
...
export function resolveExpoAndroidDevice(
  ...
  if (!ANDROID_SERIAL_RE.test(exactDeviceId)) {
    throw expoAndroidDeviceIdentityError(
      'the authority-bound adb serial is invalid; bind one exact adb serial and retry',
    );
  }
  ...
  const devices = parseAdbDevices(
    runAdb(
      ['devices', '-l'],
      'adb device enumeration failed; verify adb is available, then reconnect and authorize the exact device',
    ),
  );
  const exactRecords = devices.filter((device) => device.serial === exactDeviceId);
  if (exactRecords.length !== 1 || !isAuthorized(exactRecords[0]!)) {
    throw expoAndroidDeviceIdentityError(
      'the authority-bound adb serial is missing, offline, or unauthorized; reconnect and authorize that exact device',
    );
  }
    ...
    displayName: device.serial.startsWith('emulator-')
      ? emulatorDisplayName(
          runAdb(
            ...
            'an Expo emulator identity probe failed; verify every connected emulator has a readable AVD name',
          ),
        )
  ...
  for (const binding of bindings) {
    ...
    if (previous !== undefined && previous !== binding.deviceId) {
      throw expoAndroidDeviceIdentityError(
        'an Expo display name maps to multiple adb serials; disconnect duplicate models or AVD names and retry',
      );
    }
  ...
  if (!selected || names.get(selected.displayName) !== exactDeviceId) {
    throw expoAndroidDeviceIdentityError(
      'the Expo display name does not map uniquely to the authority-bound adb serial',
    );
  }
  return selected;
}

export function assertStableExpoAndroidDevice(
  initial: ExpoAndroidDeviceBinding,
  immediatePreSpawn: ExpoAndroidDeviceBinding,
): ExpoAndroidDeviceBinding {
  if (
    !initial.deviceId ||
    !initial.displayName ||
    ...
    throw expoAndroidDeviceIdentityError(
      'the serial-to-display-name mapping changed immediately before Expo; reconnect the exact device and retry',
    );
  }
