...
export class SessionRegistry {
  ...
  clearAutomationDutyDuringClose(session: SessionRef): void {
    const now = this.#now();
    this.#transaction(() => {
      const row = asSession(
        this.#database
      ...
      if (!row || row.state !== 'closing' || row.claim_epoch !== session.claimEpoch) {
        throw new SessionAuthorityError(
          'SESSION_OWNER_LOST',
          'only the unchanged closing session may clear its automation duty',
        );
      }
      const bindings = JSON.parse(String(row.bindings_json)) as Record<string, unknown>;
