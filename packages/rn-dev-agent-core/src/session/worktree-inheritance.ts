...
export type ResourceId = 'rn-agent-actions' | 'claude-local-md';
...
export type ResourceAction = 'none' | 'link' | 'repair';
...
function planResource(layout: WorktreeLayout, resource: ResourceSpec): ResourcePlan {
  ...
  const parentLinked =
    resource.parent !== undefined && isSymlink(join(anchor.local, resource.parent));
  const base = {
    id: resource.id,
