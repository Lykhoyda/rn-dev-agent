...
export interface SessionRuntimeAbsenceDependencies {
  resolveBinding(): SessionRuntimeBinding | null;
  listTargets(metroPort: number): Promise<{ port: number; targets: readonly HermesTarget[] }>;
  execute(
    file: string,
    args: string[],
...
function stdoutText(output: { stdout: string | Buffer }): string {
  return typeof output.stdout === 'string' ? output.stdout : output.stdout.toString('utf8');
}

async function isSessionAppRunning(
  binding: SessionRuntimeBinding,
  dependencies: SessionRuntimeAbsenceDependencies,
): Promise<boolean> {
  ...
  try {
    const output = await dependencies.execute(
      'adb',
      buildAndroidPidofArgs(binding.appId, binding.deviceId),
      { timeout: ANDROID_APP_PROBE_TIMEOUT_MS },
    );
    ...
    if (failure.code === 1 && failure.killed !== true) return false;
    throw error;
  }
}

async function observeSessionRuntimeAbsent(
  dependencies: SessionRuntimeAbsenceDependencies,
): Promise<boolean> {
  const binding = dependencies.resolveBinding();
  if (!binding) throw new InconclusiveRuntimeProbeError('session runtime binding is unresolved');
  ...
  const advertised = listed.targets.some((candidate) =>
    targetMatchesSession(candidate, { platform: binding.platform, bundleId: binding.appId }),
  );
 ...
 */
export function createSessionRuntimeAbsenceProbe(
  dependencies: SessionRuntimeAbsenceDependencies,
): () => Promise<boolean> {
  const wait =
  ...
  return async () => {
    try {
      if (!(await observeSessionRuntimeAbsent(dependencies))) return false;
