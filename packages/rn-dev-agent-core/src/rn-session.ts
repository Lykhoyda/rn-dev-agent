...
function removeSessionAndroidMetroReverse(status: ReturnType<typeof resolveStatus>): void {
  const binding = status.bindings.androidMetroReverse as
    | AndroidMetroReverseBinding
    | null
    | undefined;
  if (binding) removeAndroidMetroReverse(binding);
}
...
async function ensureManagedMetro(status: ReturnType<typeof resolveStatus>): Promise<{
  ...
  try {
    await status.registry.runWithOperation(operation, async () => {
      ...
      const restart = resolveManagedMetroRestartGeneration({
        ...
        fallbackGeneration:
          Math.max(
            ...
            Number(install?.buildGeneration ?? 0),
          ) + 1,
      });
      ...
      if (!receiptPreserved) {
        const recovery =
          restart.reason === 'install-capture-failed'
        ...
        process.stderr.write(
          `rn-session ensure-metro: install receipt generation not preserved (${restart.reason}); pin_dev_client will refuse until the receipt and Metro generations agree — ${recovery}\n`,
        );
      }
      writeMarker(status, {
        platform,
    ...
    if (command === 'ensure-metro') {
      ...
      process.stdout.write(
        `${JSON.stringify({
          ...
          buildGeneration: (current?.bindings.metro as { buildGeneration?: unknown } | undefined)
            ?.buildGeneration,
