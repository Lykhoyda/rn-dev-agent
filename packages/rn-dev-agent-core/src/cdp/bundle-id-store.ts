 ...
 * that triggered it.
 */
export function persistLastBundleId(
  platform: string,
  bundleId: string,
  projectRoot: string | null = findProjectRoot(),
): void {
  if (!projectRoot || !isValidBundleId(bundleId)) return;
  try {
 ...
 */
export function loadPersistedBundleId(
  platform: string,
  projectRoot: string | null = findProjectRoot(),
): string | null {
  if (!projectRoot) return null;
  const entry = readStore(projectRoot)[platform.toLowerCase()];
  if (!entry || typeof entry.bundleId !== 'string') return null;
