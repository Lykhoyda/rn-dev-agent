...
export interface RepairActionArgs {
  ...
  actionId: string;
  /**
   * The selector that failed. If known from a prior maestro_run failure,
   * pass it. If absent, the engine extracts all `id: "X"` selectors from
   * the body and tries to match each against current testIDs (more
   * expensive; future enhancement).
   */
   ...
   */
  projectRoot?: string;
  /** Override the fuzzy-match threshold (0..1). Default 0.6. */
  threshold?: number;
  /**
   * Don't write changes — return the diff that WOULD be applied. Useful
   * for previewing repairs before committing.
   */
  dryRun?: boolean;
  /**
   * Free-form one-liner the agent records in the RepairRecord
   * (`agentReasoning` field). Helps audit "why did this repair happen".
   */
...
export function createRepairActionHandler() {
  return withSession(async (args: RepairActionArgs): Promise<ToolResult> => {
    ...
    if (!args.failedSelector || typeof args.failedSelector !== 'string') {
      ...
      return failResult(
        'cdp_repair_action requires failedSelector — pass the testID that the prior maestro_run reported as missing',
        'BAD_FILENAME',
        {
    ...
    if (actionWasEditedExternally(action)) {
      return failResult(
        `cdp_repair_action: action "${args.actionId}" YAML mtime is newer than the agent's last write — refusing to repair. A human likely edited it; reconcile manually before re-running.`,
        'STALE_TARGET',
        {
    ...
    if (!repairBudgetAvailable(action.state)) {
      return failResult(
        `cdp_repair_action: action "${args.actionId}" exhausted its 24h repair budget — refusing to repair. The corpus is signaling churn that needs human attention.`,
        'STALE_TARGET',
        {
          actionId: args.actionId,
    ...
    if (
      ...
      return failResult(
        'cdp_repair_action requires exact session platform, app, and device authority',
        'DEVICE_AUTHORITY_MISMATCH',
      );
    }
    await bringTargetAppToForeground(targetPlatform, targetBundleId, targetDeviceId);

    if (targetPlatform === 'ios' && targetBundleId) {
      await ensureFastRunner(targetDeviceId, targetBundleId);
    }
    ...
    if (snapshotEnvelopeFailed(snapEnvelope)) {
      return failResult(
        `cdp_repair_action: snapshot failed while gathering candidate testIDs for "${args.actionId}" — agent-device unreachable`,
        'SNAPSHOT_FAILED',
        {
          actionId: args.actionId,
    ...
    if (snapshotNodes && isAgentDeviceRunnerSentinel(snapshotNodes)) {
      return failResult(
        ...
        'RUNNER_LEAK',
        {
          ...
          hint:
            `Bring the target app to foreground on ${targetDeviceId} and retry. ` +
    ...
    if (candidates.length === 0) {
      return failResult(
        `cdp_repair_action: snapshot returned ok but contained 0 testIDs — cannot match any candidate`,
        'TESTID_NOT_FOUND',
        {
          actionId: args.actionId,
    ...
    const { filePath, sidecarPath } = loadContext
      ? saveActionFromContext(loadContext, repaired)
      : saveAction(repaired);
