...
export function createConnectHandler(
  ...
  return async (args: ConnectArgs): Promise<ToolResult> => {
    ...
    if (
      ...
      return failResult(
        `cdp_connect requested ${args.platform}, but the active session is bound to ${sessionFilters.platform}; refusing cross-platform fallback. This is platform affinity, not iOS UDID identity.`,
    ...
    if (
      ...
      return failResult(
        `cdp_connect requested bundleId ${args.bundleId}, but the active session is bound to ${sessionFilters.bundleId}.`,
        'TARGET_SESSION_MISMATCH',
        { deviceSession: session },
      );
    }
    const effectiveFilters: ConnectFilters = {
      ...sessionFilters,
    ...
    if (client.isConnected && !args.force) {
      ...
      const targetIdMismatch =
        typeof effectiveFilters.targetId === 'string' &&
      ...
      const bundleMismatch =
        typeof effectiveFilters.bundleId === 'string' &&
    ...
    try {
      ...
      if (!targetMatchesSession(target, effectiveFilters)) {
        ...
        throw new TargetSelectionError(
          effectiveFilters.targetId ? 'TARGET_PLATFORM_CONFLICT' : 'PLATFORM_TARGET_NOT_FOUND',
          `Connected target failed post-connect affinity validation for platform=${effectiveFilters.platform ?? 'unspecified'} bundleId=${effectiveFilters.bundleId ?? 'unspecified'}. The socket was disconnected; run cdp_targets and relaunch the requested app.`,
          target ? [target] : [],
        );
      }
      return okResult({
