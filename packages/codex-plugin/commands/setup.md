...
Only `.rn-agent/actions` is inheritable. Keep tracked corpora Git-owned. Preview and confirm `DEST_MISSING` before `apply`. `LEGACY_ROOT_LINK` is detection-only during startup, reconnect, and `apply`, even with `--allow-repair`. Explain the typed remedy, ask for a second confirmation, then run `<package-root>/rn-dev-agent-core/dist/worktree-inheritance.js repair --app-root "$APP_ROOT"`. This explicit repository-locked repair restores the real `.rn-agent.bak` as local state and replaces only `actions` with the verified shared link, preserving `integration/`, `local/`, backups, and ambiguous action corpora. Refuse foreign roots, collisions, unavailable sources, tracked recovery paths, and Git-visible private links without mutation. Explicit legacy repair supports apps rooted at the worktree root; report the typed refusal for nested app roots. Never expose source paths or action bodies.
...
The package source is `<package-root>/codex-templates/rn-agent/` and contains the
