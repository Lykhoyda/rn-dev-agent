...
| `RESOURCE_CLAIM_CONFLICT` / second-platform `DEVICE_AUTHORITY_MISMATCH` / `DEVICE_RECEIPT_INCOMPATIBLE` | `rn_session(action="status")` | A second session on this copy, or a refused platform retarget on this session | `rn-workflow` skill § "Authority bounds — one copy, one target". Do not feed `cross_platform_verify` from another session or copy |
