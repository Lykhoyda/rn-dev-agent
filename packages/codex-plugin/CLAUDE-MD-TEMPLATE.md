...
- **Automation ownership is explicit.** A genuinely foreign Maestro/XCUITest session yields sanitized `BUSY_FOREIGN_FLOW`; wait for its owner. A live plugin flow yields `BUSY_FLOW_ACTIVE`. If owned inline-Maestro teardown cannot be positively proven, tools return `AUTOMATION_CLEANUP_UNPROVEN` and preserve an authenticated recovery duty; run `rn_session(action="recover_automation", confirmed=true)`. Recovery refuses another device/session epoch or changed PID birth. CDP reads stay free and screenshots stay on the exact simctl route.
...
| `AUTOMATION_CLEANUP_UNPROVEN` | `rn_session(action="status")` | Plugin-owned process-group absence could not be confirmed | `rn_session(action="recover_automation", confirmed=true)`; changed/ambiguous identity refuses cleanup |
