...
- A fresh fenced session starts revision 1 with empty run and repair history. Sidecar histories and revisions do not cross session boundaries. Canonical lifecycle status remains shared worktree knowledge, so later sessions see tracked YAML promoted to `active` without inheriting the earlier runtime history.
