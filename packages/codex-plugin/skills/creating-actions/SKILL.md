...
- **Body (cold entry, the default)**: `launchApp: { stopApp: false }` self-bootstrap (works cold or warm, preserves login); conditional prologues via `runFlow: { when: { visible: ... } }`; `waitForAnimationToEnd` after transitions; the diagram's anchor `assertVisible` after each screen change; `scrollUntilVisible` for potentially off-screen targets.
- **Park-state actions** (start state ≠ cold start, GH #628): declare `# entry: parked` and omit `launchApp` entirely — on an Expo dev client even `stopApp: false` reloads the JS bundle and destroys the parked navigation stack. Open with a probeable pre-mutation anchor (an id-bearing `assertVisible`, `extendedWaitUntil`, or `tapOn`; read-only assertions and waits may precede it). Replay verifies that anchor read-only first and refuses `PARK_STATE_MISSING` when the park state is absent (a backgrounded app refuses with cause `app-backgrounded`); when `expectedRouteSequence` is present, its first route is checked too. Recorder-emitted parked actions seed the full route sequence when a start route was recorded. Any lifecycle command (`launchApp`/`stopApp`/`killApp`/`clearState`), including inside an inline subflow, refuses `BAD_RECORDING` before any step runs. File subflows stay supported when replay can load, expand, and inspect them completely; an uninspectable or malformed file-form `runFlow` refuses `BAD_RECORDING` with the failing reference. Drive the app to the park state before replaying — the preflight never launches, navigates, or auto-repairs for you.
...
contract: the `using-rn-dev-agent` skill section "Session ownership recovery".
...
2. **Repair refusals are intentional** — do not force past them:
   - `entry: parked`: auto-repair relaunches the app and cannot preserve the
     declared park state; re-ground the selector and edit the action manually.
