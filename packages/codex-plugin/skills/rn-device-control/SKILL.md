...
`device_fill` hard-fails ambiguous, transformed, unreadable, lost, or uncertain fills, and never types into ambient focus or falls back to raw adb input. Corrective clear-first retypes retain the same exact native target and operation token, and success is emitted only after stable exact read-back. `device_fill` never escalates to Maestro; use `$rn-dev-agent:rn-testing` separately for an explicitly authorized Maestro flow. If failure metadata says `mutation: observed|possible`, inspect current state before deciding on any new fill.
