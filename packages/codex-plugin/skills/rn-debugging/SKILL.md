...
| Step | Action | Why |
|------|--------|-----|
| 1 | Switch the immediate task to `device_*` tools (`device_press`, `device_snapshot`, `device_screenshot`, `device_fill`) | These native tools don't depend on injected JS. `device_fill` binds and verifies one exact native input without React Fiber evidence. |
