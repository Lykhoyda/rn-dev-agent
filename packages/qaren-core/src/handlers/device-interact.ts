...
// bind exactly one native input and emit success only from stable exact native verification.
...
async function readReactInputValue(
  ...
  try {
    const result = await client.evaluate('__QAREN.readInputValue(' + JSON.stringify(testID) + ')');
