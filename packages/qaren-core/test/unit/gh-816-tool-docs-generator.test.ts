...
function runGenerator(outDir, sourcePath) {
  const env = { ...process.env, QAREN_DOCS_OUT: outDir };
  if (sourcePath) env.QAREN_DOCS_SOURCE = sourcePath;
  else delete env.QAREN_DOCS_SOURCE;
...
test('#816 generated descriptions decode supported TypeScript string escapes', async () => {
  const out = fs.mkdtempSync(join(tmpdir(), 'gh816-docs-'));
  const previousOut = process.env.QAREN_DOCS_OUT;
  const previousSource = process.env.QAREN_DOCS_SOURCE;
  try {
    process.env.QAREN_DOCS_OUT = out;
    delete process.env.QAREN_DOCS_SOURCE;
    ...
    if (previousOut === undefined) delete process.env.QAREN_DOCS_OUT;
    else process.env.QAREN_DOCS_OUT = previousOut;
    if (previousSource === undefined) delete process.env.QAREN_DOCS_SOURCE;
    else process.env.QAREN_DOCS_SOURCE = previousSource;
    fs.rmSync(out, { recursive: true, force: true });
  }
});
