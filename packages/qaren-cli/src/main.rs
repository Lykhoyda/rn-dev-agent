...
fn main() -> ExitCode {
    ...
    {
        eprintln!(
            "--plan-file, --platform, --config, --device and --fresh-install are only valid for check\n{USAGE}"
        );
        return ExitCode::from(2);
    }
