...
- **Park-state actions** (start state ≠ cold start, GH #628): declare `# entry: parked` and omit `launchApp` entirely — on an Expo dev client even `stopApp: false` reloads the JS bundle and destroys the parked navigation stack. Open the body with the park anchor (an id-bearing `assertVisible`/`tapOn`). Replay verifies that anchor read-only first and refuses `PARK_STATE_MISSING` when the park state is absent (a backgrounded app refuses with cause `app-backgrounded`); any lifecycle command (`launchApp`/`stopApp`/`killApp`/`clearState`) in a parked body refuses `BAD_RECORDING` before any step runs. Subflows must be fully inspectable: readable lifecycle-free file subflows are expanded inline, while a file-form `runFlow` that cannot be loaded and expanded refuses `BAD_RECORDING`. Drive the app to the park state before replaying — the preflight never launches or navigates for you.
