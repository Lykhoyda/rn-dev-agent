...
node --version
```
**Required:** Node.js >= 22.5. Every later major, including odd-numbered releases, is supported.
If < 22 or Node 22 with minor < 5: warn the user to install a supported Node release.
...
| Excuse | Reality |
|--------|---------|
| "Node v25 is not LTS, so the plugin must reject it" | The supported runtime contract is Node >= 22.5, including odd-numbered majors. Prefer an LTS release for installation guidance, but do not reject a supported runtime. |
...
- [ ] Node.js is >= 22.5; every later major, including odd-numbered releases, is supported
