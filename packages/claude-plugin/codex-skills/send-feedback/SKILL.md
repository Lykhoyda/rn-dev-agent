---
name: send-feedback
description: "Explicit Codex workflow: Collect reviewed, sanitized rn-dev-agent feedback and create a GitHub issue only with user confirmation."
---
<!-- GENERATED by scripts/build-host-runtimes.ts from packages/shared-agent-knowledge/source-map.json. DO NOT EDIT. -->

# send-feedback

Invoke this workflow explicitly as `$rn-dev-agent:send-feedback [request text]`.
...
Read [the complete package-local workflow](../../codex-commands/send-feedback.md) before acting. Resolve that file and every helper relative to this exact `SKILL.md` path; never scan Codex caches or rely on a plugin-root environment variable. If a required `cdp` MCP tool is absent from the active task, stop and use the read-only discovery diagnosis. Do not substitute raw Maestro for rn-dev-agent strict proof.
