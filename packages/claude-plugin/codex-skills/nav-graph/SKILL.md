---
name: nav-graph
description: "Explicit Codex workflow: Extract, inspect, and query the app navigation graph — a complete map of all screens and navigators."
---
<!-- GENERATED by scripts/build-host-runtimes.ts from packages/shared-agent-knowledge/source-map.json. DO NOT EDIT. -->

# nav-graph

Invoke this workflow explicitly as `$rn-dev-agent:nav-graph [request text]`.
...
Read [the complete package-local workflow](../../codex-commands/nav-graph.md) before acting. Resolve that file and every helper relative to this exact `SKILL.md` path; never scan Codex caches or rely on a plugin-root environment variable. If a required `cdp` MCP tool is absent from the active task, stop and use the read-only discovery diagnosis. Do not substitute raw Maestro for rn-dev-agent strict proof.
