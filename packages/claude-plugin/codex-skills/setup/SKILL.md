---
name: setup
description: "Explicit Codex workflow: Onboard a Codex project with passive diagnostics, managed AGENTS.md instructions, optional nav/store instrumentation, and the rn-agent scaffold."
---
<!-- GENERATED by scripts/build-host-runtimes.ts from packages/shared-agent-knowledge/source-map.json. DO NOT EDIT. -->

# setup

Invoke this workflow explicitly as `$rn-dev-agent:setup [request text]`.
...
Read [the complete package-local workflow](../../codex-commands/setup.md) before acting. Resolve that file and every helper relative to this exact `SKILL.md` path; never scan Codex caches or rely on a plugin-root environment variable. If a required `cdp` MCP tool is absent from the active task, stop and use the read-only discovery diagnosis. Do not substitute raw Maestro for rn-dev-agent strict proof.
