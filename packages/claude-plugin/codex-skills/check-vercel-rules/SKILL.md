---
name: check-vercel-rules
description: "Explicit Codex workflow: Run the packaged Vercel Labs agent-skills rule audit on the current project."
---
<!-- GENERATED by scripts/build-host-runtimes.ts from packages/shared-agent-knowledge/source-map.json. DO NOT EDIT. -->

# check-vercel-rules

Invoke this workflow explicitly as `$rn-dev-agent:check-vercel-rules [request text]`.
...
Read [the complete package-local workflow](../../codex-commands/check-vercel-rules.md) before acting. Resolve that file and every helper relative to this exact `SKILL.md` path; never scan Codex caches or rely on a plugin-root environment variable. If a required `cdp` MCP tool is absent from the active task, stop and use the read-only discovery diagnosis. Do not substitute raw Maestro for rn-dev-agent strict proof.
