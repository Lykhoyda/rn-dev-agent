...
## Test Runner: maestro-runner floor (>= 1.1.27)
...
`>= 1.1.27` from the versioned pin-cache (attested 1.1.27 is the default
...
Success requires `pinned-ok`, version `>= 1.1.27`, provenance `pin-cache`,
...
| `enginePin` | `maestro-runner@1.1.27` or newer | required for replay; missing or older pins are terminal before UI mutation |
...
1. **ALWAYS use rn-dev-agent replay tools on Android** — they enforce the
   supported Maestro Runner floor `>= 1.1.27` and the authority-bound device.
...
| maestro-runner >= 1.1.27 | Required | YAML E2E test execution | Package pin-cache installer |
