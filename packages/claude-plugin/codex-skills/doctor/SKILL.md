---
name: doctor
description: "Explicit Codex workflow: Run strictly read-only Codex plugin, MCP contract, schema, and environment diagnostics; recommend but never execute recovery."
---
<!-- GENERATED by scripts/build-host-runtimes.ts from packages/shared-agent-knowledge/source-map.json. DO NOT EDIT. -->

# doctor

Invoke this workflow explicitly as `$rn-dev-agent:doctor [request text]`.
...
Read [the complete package-local workflow](../../codex-commands/doctor.md) before acting. Resolve that file and every helper relative to this exact `SKILL.md` path; never scan Codex caches or rely on a plugin-root environment variable. If a required `cdp` MCP tool is absent from the active task, stop and use the read-only discovery diagnosis. Do not substitute raw Maestro for rn-dev-agent strict proof.
