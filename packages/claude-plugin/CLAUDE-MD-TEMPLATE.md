...
- **XCTest's idle-wait (quiescence) is bypassed by default on iOS.** RN apps with looping animations/Reanimated worklets never report idle, which used to stall queries and snapshots. Opt out with `RN_QUIESCENCE_BYPASS=0`; audit via `meta.quiescenceBypass` on device results. `XCUIElement.typeText` runs its own internal sync, so `device_fill` may still hit a main-thread timeout. The client poisons and reaps that runner, and the fill fails closed with a possible mutation outcome; inspect current field state before deciding whether a new fill is safe.
...
| **Self-healing taps** | A stale `@ref` re-resolves inline by identity signature (unique match only — ambiguous/absent → `STALE_REF` with candidates); a tap that produced no UI change retries exactly once, unless keyboard-guard or transport recovery already consumed that single retry budget (then it re-fires nothing). On iOS a still-unchanged tap stays a success carrying `meta.noUiChange`; on Android it fails with `INTERACTION_EFFECT_UNVERIFIED` (`mutation: possible`), and so does a tap whose effect cannot be probed at all — disabling the retry does not turn that back into a success. An Android gesture the runner could not actuate fails with `INTERACTION_NOT_ACTUATED` (`mutation: none`). `device_batch` testID resolution refuses ambiguous matches (`AMBIGUOUS_TESTID`). | `meta.reResolved`, `meta.tapRetried`, `meta.noUiChange` | `RN_SELF_HEAL=0` global, `retryIfNoChange: false` per call (retry only — the Android effect check goes away only with the settle engine itself) |
