...
| Run an E2E flow | **First**: replay an inventory-matched learned action via `cdp_run_action`, following the run-action workflow for prerequisites and proof. **Then**: `device_*` for novel verification. **Last**: `maestro_generate` to persist the new flow under `.rn-agent/actions/`. | hand-rolled bash loops, ad-hoc xdotool/AppleScript, recreating a flow that already exists |
...
The fenced session owns the platform and exact device. It holds one target and
replaces it; see the `rn-workflow` skill § "Authority bounds — one copy, one
target". `cdp_status` only reports the current client; conflicting authoritative
tool arguments fail instead of silently re-targeting. Expo Android's `--device`
is the one compatibility
...
13. Two sessions on one project root, or iOS and Android on one live session — `rn-workflow` § "Authority bounds — one copy, one target" (`cross_platform_verify` cannot import another session's snapshots)
...
| `METRO_START_UNAVAILABLE` with `launcher alive at deadline; readiness deadline N ms expired` from the package script | the refusal's own `Metro log tail` (a pre-shutdown snapshot) | No launcher-owned listener was observed within the configured readiness budget (90 s by default); anything Metro printed while being stopped, such as an `EPIPE`, is a consequence, not the cause | Retry; for a project that needs longer set `.rn-agent/config.json` → `{ "metro": { "readinessTimeoutMs": 150000 } }` (integer, 1000–600000; a timeout only, a malformed value refuses with `METRO_READINESS_TIMEOUT_INVALID`). The generated adapter derives its `ensure-metro` SIGKILL bound from that budget plus 100 s of headroom for the work outside the readiness loop, so 150000 is actually waited. The first-build advisory `no-install-receipt` is expected, not a second failure |
...
| `RESOURCE_CLAIM_CONFLICT` / second-platform `DEVICE_AUTHORITY_MISMATCH` / `DEVICE_RECEIPT_INCOMPATIBLE` | `rn_session(action="status")` | A second session on this copy, or a second platform target on this session | `rn-workflow` skill § "Authority bounds — one copy, one target". Do not feed `cross_platform_verify` from another session or copy |
...
| `maestro_run` fails with `RUNTIME_DEGRADED` hint | `meta.trailingVerification` | With the qualifier, mutations completed but the goal state is unproven; without it, the simulator runtime is likely wedged | Qualified: verify the live state before rebooting. Unqualified: `xcrun simctl shutdown/boot` the simulator, relaunch, retry — don't chase app code |
...
| Exact-testID replay fails | `cdp_status` + result `proofDomain` | iOS exact React identity is proved by `react-tree`; native/system selectors stay `xctest-native` | Reconnect the exact bundle for React-tree failures. `NATIVE_SURFACE_BLIND` is emitted only with bounded native same-screen evidence; otherwise preserve the ordinary selector miss. |
