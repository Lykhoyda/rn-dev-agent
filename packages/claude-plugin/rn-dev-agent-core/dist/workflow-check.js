...
// packages/rn-dev-agent-core/dist/workflow-check.js
...
import { closeSync, constants, existsSync as existsSync2, fstatSync, lstatSync as lstatSync2, mkdtempSync, openSync, readFileSync as readFileSync2, realpathSync, renameSync as renameSync2, rmSync as rmSync2, writeFileSync as writeFileSync2 } from "node:fs";
import { tmpdir as tmpdir2 } from "node:os";
import { join as join4 } from "node:path";

// packages/rn-dev-agent-core/dist/session/state-root.js
import { randomBytes, randomUUID } from "node:crypto";
import { chmodSync, linkSync, lstatSync, mkdirSync as mkdirSync2, readFileSync, renameSync, rmSync, statSync, writeFileSync } from "node:fs";
...
function getStateDir() {
  if (process.env.XDG_STATE_HOME) {
    return join2(process.env.XDG_STATE_HOME, "rn-dev-agent");
  }
  if (process.platform === "darwin") {
    return join2(homedir2(), "Library", "Application Support", "rn-dev-agent");
  }
  return join2(homedir2(), ".rn-dev-agent");
}
...
function authorityStateLayout(stateDir) {
  ...
  const root = join3(resolvedStateDir, "v2");
  return {
    root,
    registry: join3(root, "registry.sqlite3"),
    sessions: join3(root, "sessions"),
    runners: join3(root, "runner"),
    observe: join3(root, "observe"),
    migrations: join3(root, "migrations")
  };
}
...
function getBoundDirectoryJournalKey(layout = createAuthorityStateLayout()) {
  const path2 = join3(layout.root, "bound-directory.key");
  const temporary = join3(layout.root, `.bound-directory.${randomUUID()}.key`);
  try {
    try {
      writeFileSync(temporary, randomBytes(32), { flag: "wx", mode: 384, flush: true });
      try {
...
function waitForFile(path2, timeoutMs) {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
...
function stopWorker(worker, signal = "SIGTERM") {
  const stoppedPath = join4(worker.controlPath, "stopped");
  if (signal === "SIGTERM") {
    try {
      writeFileSync2(join4(worker.controlPath, "stop"), "", { flag: "wx", mode: 384 });
    } catch {
    }
    if (waitForFile(stoppedPath, 1e3)) {
      if (!existsSync2(join4(worker.controlPath, "lock-retained"))) {
  ...
  try {
    writeFileSync2(join4(worker.controlPath, "terminate"), JSON.stringify({
...
function bindWorker(controlPath, child, owner, childId, lifecycleCapability = "") {
  ...
  const readyPath = join4(controlPath, "ready");
  ...
  let ready = {};
  try {
    ready = JSON.parse(readFileSync2(readyPath, "utf8"));
  } catch {
    rejectWorker("SESSION_INTEGRATION_PATH_UNSAFE: bound-directory worker unavailable");
  }
...
function startWorker(path2, identity, realPath) {
  const controlPath = mkdtempSync(join4(tmpdir2(), "rn-bound-directory-"));
...
function startSubdirectoryWorker(parent, name, expectedIdentity, expectedRealPath) {
  ...
  try {
    const result = runBoundOperation(parent, {
      ...
      lifecycleCapability,
      name,
      publicPath: join4(parent.path, name),
      create: false,
      mode: 448
    });
    childStarted = true;
...
function sendOperation(directory, request, timeoutMs) {
  ...
  const pendingPath = join4(directory.worker.controlPath, `${prefix}.pending`);
  const requestPath = join4(directory.worker.controlPath, `${prefix}.request`);
  const responsePath = join4(directory.worker.controlPath, `${prefix}.response`);
  ...
  let result;
  try {
    result = JSON.parse(readFileSync2(responsePath, "utf8"));
  } catch {
...
function runBoundOperation(directory, request, dependencies = {}) {
  ...
  let currentRealPath;
  try {
    current = lstatSync2(directory.path, { bigint: true });
...
function openValidatedDirectory(path2, expected) {
  let descriptor;
  let worker;
  try {
    const before = lstatSync2(path2, { bigint: true });
    if (!before.isDirectory() || before.isSymbolicLink()) {
      throw new Error("SESSION_INTEGRATION_PATH_UNSAFE: integration ancestor is not a directory");
    }
    descriptor = openSync(path2, constants.O_RDONLY | (constants.O_DIRECTORY ?? 0) | (constants.O_NOFOLLOW ?? 0));
    const opened = fstatSync(descriptor, { bigint: true });
    const after = lstatSync2(path2, { bigint: true });
    const realPath = realpathSync(path2);
    ...
    worker = startWorker(path2, identity, realPath);
    const directory = {
...
function openBoundDirectory(path2) {
  return openValidatedDirectory(path2);
}
...
function stateRootFacts() {
  const kind = process.env.XDG_STATE_HOME ? "xdg" : process.platform === "darwin" ? "darwin" : "home";
...
function preflight(projectRoot) {
  ...
  const workspaceManifest = workspaceRoot === projectRoot ? packageJson : readTextIfFile(path.join(workspaceRoot, "package.json")) ?? packageJson;
...
function readSessionStatus(statusFile) {
  ...
  if (raw === null)
    fail2(2, "workflow-check: --status-file is missing or unreadable");
  let parsed;
  try {
    parsed = JSON.parse(raw);
  } catch {
    return fail2(2, "workflow-check: --status-file is not valid JSON");
  }
  ...
  const isCanonical = isRecord(parsed) && parsed.ok === true && authority !== null && typeof authority.state === "string" && runtime !== null && typeof runtime.metroBound === "boolean" && automation !== null && typeof automation.runnerBound === "boolean" && typeof automation.recorderBound === "boolean";
  if (!isCanonical) {
    return {
      session: {
        provided: true,
...
function postflight(projectRoot, statusFile) {
  ...
  try {
    integrationMarkersPresent = inspectPackageIntegrationFileState(projectRoot).verdict !== "unintegrated";
  } catch {
  ...
  const facts = {
    ...
    recordingResidue,
    cleanupProven: status.stop === null && status.session.provided && status.session.metroBound === false && status.session.runnerBound === false && status.session.recorderClaim === false,
    stateRoot: stateRootFacts(),
...
function parseArgs(argv) {
  ...
  if (mode !== "preflight" && mode !== "postflight") {
    fail2(2, 'workflow-check: first argument must be "preflight" or "postflight"');
  }
  ...
  for (let i = 1; i < argv.length; i += 1) {
    ...
    if (arg === "--project" && argv[i + 1]) {
      ...
      fail2(2, `workflow-check: unknown argument ${arg}`);
    }
  }
