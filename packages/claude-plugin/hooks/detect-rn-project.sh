...
if [ "$has_rn_config" = true ]; then
  ...
  # Warn if Node.js is below the supported floor.
  ...
  if ! bash "$SCRIPT_ROOT/ensure-maestro-runner.sh" --print-bin >/dev/null; then
    INSTALL_WARNINGS+=("WARNING: attested maestro-runner 1.1.27 is missing from the pin-cache (floor >= 1.1.27). Install it explicitly (SessionStart never downloads): bash ${SCRIPT_ROOT}/ensure-maestro-runner.sh")
  fi
