...
if [ "$has_rn_config" = true ]; then
  ...
  PROBE_OUT=$(node "$SCRIPT_ROOT/mcp-bridge-probe.mjs" --plugin-root "$PLUGIN_ROOT" --core-root "$CORE_ROOT" --upgraded "$UPGRADE_DETECTED" 2>/dev/null || true)
  if [ -n "$PROBE_OUT" ]; then
    echo "$PROBE_OUT"
    echo ""
  fi
  ...
  if ! bash "$SCRIPT_ROOT/ensure-cdp-deps.sh" 2>&1; then
  ...
  if ! bash "$SCRIPT_ROOT/ensure-idb-companion.sh" 2>&1; then
    INSTALL_WARNINGS+=("WARNING: opt-in idb-companion install failed. Run: brew tap facebook/fb && brew trust facebook/fb && brew install idb-companion (required for physical iOS device automation)")
  fi

  # Ensure ffmpeg for video-to-GIF conversion (optional — not critical)
  bash "$SCRIPT_ROOT/ensure-ffmpeg.sh" 2>/dev/null || true
  ...
  bash "$SCRIPT_ROOT/ensure-idb.sh" 2>/dev/null || true

  # Linked-worktree private context (GH #357). REPORT ONLY: a symlink created
  # here is not loaded by the session that created it, and repairs touch user
  # state, so both belong to /rn-dev-agent:setup. Bounded (GH #252): a few git
  # plumbing calls and lstats, no network, no install.
  if command -v node >/dev/null 2>&1 && [ -f "$CORE_ROOT/dist/worktree-inheritance.js" ]; then
    if WORKTREE_REPORT=$(node "$CORE_ROOT/dist/worktree-inheritance.js" report \
      ...
      echo "NOTICE: rn-dev-agent could not check linked-worktree private context. Run /rn-dev-agent:doctor."
      echo ""
    fi
  else
