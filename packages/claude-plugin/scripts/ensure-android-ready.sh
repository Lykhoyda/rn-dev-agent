...
# 4. Require the attested pin-cache maestro-runner (floor >= 1.1.27, SHA256). Never PATH,
...
if [ ! -f "$ENSURE" ]; then
  ...
  errors+=("attested maestro-runner >= 1.1.27 required (version + checksum). Supported correction: bash $ENSURE")
fi
