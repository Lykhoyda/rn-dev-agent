...
if [ ! -d "$CDP_DIR" ]; then
  # Package copy of this script (claude-plugin/scripts): the sibling is the
  # bundled host runtime emitted by scripts/build-host-runtimes.ts.
  CDP_DIR="$SCRIPT_DIR/../rn-dev-agent-core"
fi
if [ ! -d "$CDP_DIR" ]; then
  CDP_DIR="$SCRIPT_DIR/cdp-bridge"
fi
# Bundled host runtime: fully self-contained esbuild output with no runtime
# dependencies — nothing to install, and npm must not run inside it.
if [ -f "$CDP_DIR/package.json" ] && ! grep -q '"dependencies"' "$CDP_DIR/package.json"; then
  exit 0
fi
