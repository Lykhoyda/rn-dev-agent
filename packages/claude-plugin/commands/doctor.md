---
command: doctor
description: Diagnose installation health. Check Node, CDP bridge, rn-fast-runner (iOS), rn-android-runner (Android), maestro-runner, learned-action compatibility, simulators, Metro, CDP, injected helpers, ffmpeg, physical devices, plugin version, Vercel rules sync. Reports what's missing — does NOT modify your project.
argument-hint: 
---

Run the environment-diagnostic checklist from the `rn-setup` skill. Walk all 18 prerequisite checks (Node.js version, CDP bridge dependencies, **rn-fast-runner build (iOS)**, **rn-android-runner build/install (Android)**, maestro-runner, **learned-action compatibility**, iOS simulator, Android emulator, Metro dev server, CDP connection, **injected `__RN_AGENT` helpers**, ffmpeg, **idb (screen-mirror fast path)**, physical-device prerequisites, **plugin version freshness**, **Vercel rules sync freshness**, **CDP auto-reconnect mode**, **linked-worktree action inheritance**) and surface install commands for any missing dependencies.
...
Present results as an 18-row table. Follow the detailed probes and remediation
...
For **learned-action compatibility**, resolve the RN app root and run the packaged `maestro-runner-pin.js diagnose-actions --root <app> --json`. Print one table row with `scanned`, `compatible`, and the `enginePin` / `regexSelector` / `unreadable` counts plus those action ids. A `pinned-ok` runner with any of those refusals is FAIL (or YELLOW when the only findings are `unreadable`), never silent GREEN. Inherited symlink corpora may be scanned. Never run `migrate-actions`, rewrite an action, print a YAML body, or print a private inherit source path from doctor.
