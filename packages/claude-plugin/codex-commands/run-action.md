...
1. **Parse arguments.** The first positional is the required action name.
   Recognize `--platform ios|android`, `--no-auto-repair`, and
...
3. **Pre-flight safety checks** before replay:
   ...
   - **Read the flow file** and look for a `mutates: true` or
     ...
     `codex-skills/rn-testing/SKILL.md` § "Reusable Action Metadata Schema (M7)"). If present,
...
6. **Execute via MCP**:
   ...
   `packages/rn-dev-agent-core/src/tools/run-action.ts`):
   ```
   {
     ok: true | false,
     data: {
...
rather than literally invoking `$rn-dev-agent:run-action`, but the contract is
the same.
...
See `codex-commands/list-learned-actions.md` for the discovery counterpart and
`packages/rn-dev-agent-core/src/learned-actions.ts` (compiled to `packages/rn-dev-agent-core/dist/learned-actions.js`) for the underlying script (single source of
