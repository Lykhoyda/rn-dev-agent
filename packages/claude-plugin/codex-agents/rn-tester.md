...
if [ -z "$CODEX_PLUGIN_ROOT" ] && [ -f "packages/claude-plugin/.codex-plugin/plugin.json" ]; then
  CODEX_PLUGIN_ROOT="packages/claude-plugin"
fi
...
8. **Verification fidelity — no silent shortcuts** (GH #61): Verification
   ...
   `codex-skills/rn-testing` → "Verification Fidelity" for legitimate exceptions
