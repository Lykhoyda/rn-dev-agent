...
- Platform-specific device-control runtime (one of):
  ...
  - **Android** — in-tree `rn-android-runner` (Kotlin/UIAutomator2, `scripts/rn-android-runner/`) ships with the plugin and is the sole Android device backend (the third-party `agent-device` dependency was removed). Built once with `./gradlew :app:assembleDebug :app:assembleDebugAndroidTest`, then launched via `adb shell am instrument` with the host port `adb forward`-mapped. Requires the Android SDK + a running emulator/device.
...
- **"agent-device not installed"** → No longer relevant: the `agent-device` dependency was removed. Android uses the in-tree `rn-android-runner`, iOS the in-tree `rn-fast-runner` — neither needs `agent-device`. If a `device_*` call returns `RN_ANDROID_RUNNER_DOWN`/`RN_FAST_RUNNER_DOWN`, the in-tree runner couldn't start (check the emulator/simulator + SDK), not a missing CLI.
...
| Device interaction (Android) | In-tree `rn-android-runner` (Kotlin/UIAutomator2, `scripts/rn-android-runner/`) — NanoHTTPD `POST /command`, host port via `adb forward` | Native Android device control: tap, swipe, fill, find, snapshot, screenshot |
...
Android dispatch routes through the in-tree `rn-android-runner` (`runAndroid()` → NanoHTTPD `POST /command`); the dispatcher short-circuits to it (with an ensure-runner pre-flight mirroring iOS) and `runNative()` has no `agent-device` daemon/CLI tiers (eradicate-agent-device). A foreign/stale `AgentDeviceRunner` is still detected at session-open on iOS too and, since #202, terminated by default — `ensureSingleRunner()` kills stale `AgentDeviceRunner` processes scoped to the target simulator UDID and clears orphaned `~/.agent-device/daemon.{json,lock}` (opt out with `RN_DEVICE_KILL_LEGACY=0`) — because a stale daemon respawns the upstream `AgentDeviceRunner` and fights our `RnFastRunner` for focus. Phase 4 (#202) extends this to the installed apps themselves: at device-open, `ensureSingleRunner` also `simctl uninstall`s the legacy `com.callstack.agentdevice.runner{,.uitests.xctrunner}` bundles (scanned at every open — one `simctl listapps`, ~150–350 ms measured; same `RN_DEVICE_KILL_LEGACY=0` opt-out), because killing processes can't stop iOS relaunching an installed XCUITest runner mid-flow.
...
| **L2 INTERACTION** | iOS `RnFastRunner` / Android `rn-android-runner`; `cdp_interact` | primitive taps / types / scrolls | **shared** | re-attach, don't evict (Tier-0 reacquire + CDP re-pin, #188) |
...
**Device tools** (14, native interaction — iOS: in-tree `rn-fast-runner` `/command`; Android: in-tree `rn-android-runner` `/command`):
...
- iOS device verbs route through `rn-fast-runner-client.ts` (`runIOS()` → `/command`); Android verbs route through `rn-android-runner-client.ts` (`runAndroid()` → `/command`). Both dispatch via `runNative()` in `agent-device-wrapper.ts` (the module name is historical); the `agent-device` daemon/CLI tiers were deleted — D1219/PR #164 (iOS), eradicate-agent-device (Android)
