...
|-------|------|------|
| **Device interaction (iOS)** | In-tree `rn-fast-runner` XCTest rig (`scripts/rn-fast-runner/`) — `POST /command` HTTP | Native iOS device control. Always calls `XCUIApplication.activate()` per request (D1219, PR #164). The in-tree runner is the sole iOS device backend. |
| **Device interaction (Android)** | In-tree `rn-android-runner` (`scripts/rn-android-runner/`) — UiAutomator instrumentation via `adb shell am instrument` | Native Android device control. The in-tree runner is the sole Android device backend (default-on; `RN_ANDROID_RUNNER=0` errors, it does not fall back). |
