...
In passive mode the bridge **yields the seat** and reconnects only when a CDP tool actually runs (e.g. `cdp_status`, `cdp_component_tree`, any `device_*` call). Once you reopen React Native DevTools after a tool call, the bridge yields again automatically.
