...
| `device_fill` reports `RUNNER_TIMEOUT` | XCTest timed out on a non-cancellable type operation. The runner is poisoned/reaped; success is returned only when exact independent CDP readback proves the requested value (`meta.runnerTimeoutRecovery`). Otherwise reconnect CDP and retry from a fresh snapshot. |
