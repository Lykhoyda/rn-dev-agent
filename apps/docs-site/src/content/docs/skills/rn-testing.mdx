...
Before testing, check `cdp_navigation_state` for auth screens. If logged out, call `cdp_login_prologue`, which inventories and replays only the exact `user-login` action and requires a fresh passing RunRecord. A failed replay returns its own failure code and latches nothing, but the journey stops there: do not fall back to credentials, `cdp_auto_login`, ad-hoc Maestro, navigation shortcuts, or store mutation. The passing helper is not PR proof; lock and run the exact login e2e candidate for formal proof.
