...
artifact, and reports the retained path after a successful capture. A supplied
directory must be owned by the current user, use mode `0700`, and not be a
symlink. Concurrent captures targeting the same output directory fail closed
while its owner-controlled lock is held.
