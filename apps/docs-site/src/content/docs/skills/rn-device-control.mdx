...
- **iOS** — fresh-geometry, dismiss-first. The keyboard frame is intersected with the target's **fresh** rectangle (validated against the current `snapshotGeneration`). If the geometry is stale or unknown while a keyboard is visible, the keyboard is always dismissed first, the snapshot/ref is refreshed, and the tap is retried once. Dismissal tiers are native swipe, native dismiss control, then injected `Keyboard.dismiss()` when CDP is connected; only when every tier fails is the tap **refused** with `KEYBOARD_DISMISS_FAILED` — no blind tap is ever dispatched under a visible keyboard.
