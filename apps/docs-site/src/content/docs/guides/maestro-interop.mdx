...
When an L2 call sees the runner-leak sentinel after a maestro run, the device-session performs a **state-preserving reacquire** — it re-foregrounds your app (no relaunch) and marks CDP stale so the next read transparently reconnects. If recovery escalates to attach-only or full relaunch, it still reopens the exact authority-bound device instead of selecting an ambient simulator. You do **not** need a manual `cdp_status` re-pin (shipped in #188).
...
**Division of labor:** rn-dev-agent's `maestro_run` is the canonical Maestro surface — it participates in the device arbiter, parks the L2 interaction runner for the flow, forwards a matching active session as an exact `--device` / `--udid` target, verifies maestro-runner's direct selected-device and iOS WDA evidence, marks CDP stale afterwards, and auto-repairs saved actions. A mismatch fails as `DEVICE_AUTHORITY_MISMATCH`; requested metadata alone is never replay proof. Use the standalone maestro-mcp for ad-hoc exploration; when both target one simulator, rn-dev-agent now steps back cleanly instead of fighting.
