...
`/rn-dev-agent:doctor` reports the resolved mode as the **CDP auto-reconnect**
row (YELLOW when off — informational, not an error). The environment variable
takes precedence over `.rn-agent/config.json`; otherwise the default is on.
...
| To check which mode is active | `/rn-dev-agent:doctor` |
