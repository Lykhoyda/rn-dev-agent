...
7. `prepare_handoff` / `accept_handoff` transfer ownership explicitly, but never adopt a live runner, Observe server, or managed Metro. Acceptance durably records and proves each source-resource shutdown before completing; interrupted cleanup remains resumable only by re-presenting the exact consumed handoff ID and its original capability token, which the registry rebinds to the cleanup lifecycle and revalidates non-mutatingly — together with the target session, claim epoch, and currently authorized cleanup worker — before any donor lookup, diagnostics, or cleanup mutation, so resumption stays available to the current fenced recovery worker, including after a worker respawn, or through a later conservatively authorized stale adoption. When package integration is installed, the recipient becomes its restoration owner.
