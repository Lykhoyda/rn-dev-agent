...
9. `restore_integration` with `confirmed: true` restores the package scripts and Metro config only after runtime authority is absent. A successful `apply_integration` also stores the restoration manifest inside the durable session binding, so restoration stays byte-exact even if the on-disk manifest copy is lost across crashes or adoptions. If a transferred legacy binding has no recoverable manifest anywhere, `restore_integration` reconciles it by clearing the binding only when deterministic markers prove `package.json`, the Metro config, and `.rn-agent/integration/` carry no integration; integrated, partially integrated, or ambiguous files refuse with the exact markers and the supported next action instead of guessing a baseline. `adopt_stale` applies the same rule up front: it transfers a restorable binding and reports the restoration duty, reconciles a provably absent legacy integration, or refuses adoption before creating a lifecycle it cannot recover, and `status` exposes any active binding fence with its owner, manifest availability, and next supported action.
