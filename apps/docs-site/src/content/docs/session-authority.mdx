...
The strict dirty digest covers tracked changes, every untracked file, and every ignored file except dependency stores and native build/cache roots such as `node_modules`, CocoaPods, Gradle, Expo, and platform build outputs. Ignored configuration, generated source, and assets remain authoritative inputs; proof fails closed when their bounded file-count or byte budget is exceeded.
