...
8. `release` cancels any interrupted fenced operation, then releases claims. A managed Metro must first stop with its HMAC-authenticated management proof and exact launcher, listener, and port identities; failed cleanup leaves the session claimed.
