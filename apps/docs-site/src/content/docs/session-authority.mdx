...
The strict dirty digest covers tracked changes, every untracked file, and every ignored file except native build/cache roots such as CocoaPods, Gradle, Expo, and platform build outputs. Dependency stores are accepted only when Git authenticates their contents; an ignored `node_modules`, Yarn cache, or unplugged store makes strict proof fail closed. Ignored configuration, generated source, and assets remain authoritative inputs; proof also fails closed when their bounded file-count or byte budget is exceeded.
