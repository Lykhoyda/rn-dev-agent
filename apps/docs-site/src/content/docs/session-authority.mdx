...
The strict dirty digest covers tracked changes, every untracked file, and every ignored file except native build/cache roots such as CocoaPods, Gradle, Expo, and platform build outputs. Local `node_modules`, Yarn cache, and unplugged dependency stores are streamed into the digest with limits of 50,000 entries, 128 MB per file, 512 MB total, and 128 path levels. Symlink targets are included even when their bytes live outside the worktree. Plug’n’Play, ancestor or filesystem-root `node_modules`, external `NODE_PATH`, external Metro watch/module roots, and custom Metro resolvers fail closed. Ignored configuration, generated source, and assets remain authoritative inputs; proof also fails closed when their separate bounded file-count or byte budget is exceeded.
