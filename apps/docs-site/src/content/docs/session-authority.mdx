...
The strict dirty digest covers tracked changes, every untracked file, and every ignored file except native build/cache roots such as CocoaPods, Gradle, Expo, and platform build outputs. Local `node_modules`, Yarn cache, unplugged dependency stores, and signed Metro runtime-policy inputs are streamed into the digest with limits of 50,000 entries, 128 MB per file, 512 MB total, and 128 path levels. This includes symlink targets and external resolver roots. Strict proof requires Metro started by the managed launcher, Node.js 22.15 or newer, and one exact terminal integration block. Bundle-source modules may resolve from Git-authenticated source or local dependency stores. Only Metro's standard transform worker, Babel transformer, and minifier executable closures are accepted; custom workers, extractors, haste implementations, and asset plugins fail closed. Runtime-policy paths and violations accumulate monotonically, module loading is observed for both CommonJS and ESM, and the policy refreshes after successful or rejected serializer and transform-option callbacks. Plug’n’Play, ancestor or filesystem-root `node_modules`, custom resolvers or serializers, external or excluded executable modules, unresolvable executable modules, and `NODE_OPTIONS` loaders fail closed. Ignored configuration, generated source, and assets remain authoritative inputs; proof also fails closed when their separate bounded file-count or byte budget is exceeded.
