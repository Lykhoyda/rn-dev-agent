...
Inline Maestro runs use one detached process group. Timeout and failure cleanup sends bounded SIGTERM, escalates to SIGKILL, and confirms group absence before returning. If absence cannot be proven, `AUTOMATION_CLEANUP_UNPROVEN` preserves the authenticated session/claim/device/PID-birth duty in the existing private runner-state area. `rn_session(action="recover_automation", confirmed=true)` may clean only that exact duty and refuses a changed birth, another device/session epoch, or genuinely foreign automation. Session transitions remain fenced until recovery succeeds.
