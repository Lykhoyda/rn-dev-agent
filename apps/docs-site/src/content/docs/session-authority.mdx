...
5. Literal `pnpm ios` or `pnpm android` starts or validates package-local Metro on the allocated port, injects the exact device and native-client port, routes Expo Dev Client launch through that managed endpoint even when an ambient default-port Metro exists, and records the installed artifact receipt.
