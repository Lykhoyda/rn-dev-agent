...
Maestro-backed flows, including action replay, E2E lock, and E2E suite paths, transactionally park the bound native runner before dispatch. A `launchApp` stage is then followed by a managed relaunch of the exact bound app against the session's allocated Metro port; replay continues only after that exact device target re-registers. Nested replay paths inherit the same authority callbacks, so they cannot fall back to an ambient Metro. Once parked, runner authority is deliberately absent from postflight and receipt axes even when replay is refused, so a stale runner binding cannot survive the attempt.
