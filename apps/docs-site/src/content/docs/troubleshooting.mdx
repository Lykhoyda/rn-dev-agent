...
`maestro_run`/`cdp_run_action` forwarded the active session's exact device
ID (`--device`/`--udid`) but maestro-runner's own report/log did not prove it
ran there: the selected device was missing, ambiguous, different from the
request, or an iOS WDA build/start line named another device. The replay is
refused even when the runner exited 0, because requested metadata is never
execution proof — `meta.deviceAuthority` carries the observed IDs and the
exact reason. Re-open the device session on the intended simulator/emulator
(`device_snapshot action=open`), shut down stray devices, and retry; pass
`deviceId` explicitly to `maestro_run` to pin the target.
