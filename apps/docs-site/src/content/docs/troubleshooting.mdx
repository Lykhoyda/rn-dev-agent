...
| `METRO_LAUNCHER_ENVIRONMENT_INVALID`, `METRO_LAUNCHER_EVIDENCE_UNAVAILABLE`, `METRO_LAUNCHER_ENFORCEMENT_REFUSED`, `METRO_LAUNCHER_CHILD_SPAWN_FAILED`, `METRO_LAUNCHER_PRE_EVIDENCE_FAILED` | Managed Metro failed before authoritative runtime evidence was available. The bounded diagnostic names the failed stage without exposing credentials or local paths; repair that stage, then retry the literal package script. |
