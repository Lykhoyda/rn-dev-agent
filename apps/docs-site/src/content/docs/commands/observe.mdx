...
- The event timeline lives in a small bounded in-memory ring buffer — the observability stream itself never touches disk. (Action replays write RunRecords to the fenced session's own state directory; an unfenced compatibility process uses `.rn-agent/state/`. Locked E2E replay output remains under `.rn-agent/e2e/`.)
