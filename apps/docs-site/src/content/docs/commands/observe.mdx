...
An autostarted UI holds the session's device axis, so the first `rn_session(action="bind_device")` stops it to yield that axis and reports `observeYielded` with the stopped port — bring it back with `restart`, but a restarted UI counts as one you started, so it forfeits the automatic yield and the next `bind_device` refuses until you stop it yourself. A UI you started yourself is never stopped for you either: that `bind_device` refuses instead, and you stop it. See [parallel session authority](../../session-authority/).
