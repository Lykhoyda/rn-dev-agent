...
- Up to 200 typed runner lifecycle events in a 256 KB bundle, with candidate and runner-pin identity, release commit when available, platform/OS/runtime, action ID, own Metro port, a redacted session ID, and an evidence-root-salted stable device-ID hash
