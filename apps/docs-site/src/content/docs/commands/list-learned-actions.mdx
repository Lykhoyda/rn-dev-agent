...
| **B. Actions** | Saved flows available for read-only discovery | `.rn-agent/actions/<name>.yaml` |
...
Listing an action proves only that its file exists. Before replay, `/run-action` reads `rn_session({ action: "status" })`; a blocked session must follow the measured recovery in [parallel session authority](../../session-authority/).
