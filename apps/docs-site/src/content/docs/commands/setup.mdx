...
SessionStart only reports missing or unsafe inheritance. Doctor inspects the
same state read-only; setup owns every link, legacy migration, and hook change.
