...
5. **Stop recording** — end video, normalize to 30 fps, convert to GIF
...
- ffmpeg for 30 fps cadence normalization, GIF conversion, and video labeling
  (`brew install ffmpeg`); without it an iOS capture stays a native `.mov` and
  strict proof rejects it, while Android keeps a native `.mp4` whose sparse
  frame timing may play as a slideshow
