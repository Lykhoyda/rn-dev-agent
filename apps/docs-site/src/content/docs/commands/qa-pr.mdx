...
| 6 | Feature proof: usable screen, real path recorded, action reused (proof replay, no writes) or saved without `launchApp`, a reused action's first rehearsal from the screen the app is on, return to the first screen off camera before a saved action's first rehearsal, a repeat rehearsal, and the take, off-camera rehearsal, no runtime reset or relaunch under the recorder, video started before the runner, on-camera `cdp_run_action` proof replay, validation |
