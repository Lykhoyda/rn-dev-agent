...
| 6 | Feature proof: usable screen, real path recorded, action reused (proof replay, no writes) or saved, off-camera rehearsal, return to the first screen off camera, no runtime reset or relaunch under the recorder, video started before the runner, on-camera `cdp_run_action` proof replay, validation |
