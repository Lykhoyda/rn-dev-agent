...
| 6 | Feature proof: usable screen, real path recorded, action reused (proof replay, no writes) or saved, off-camera rehearsal, reset with `cdp_reload`, video started before the runner, `maestro_run` take, validation |
