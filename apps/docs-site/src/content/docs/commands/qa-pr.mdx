...
| 6 | Feature proof: usable screen, real path recorded, action reused (proof replay, no writes) or saved, off-camera rehearsal, no runtime reset, video started before the runner, on-camera `cdp_run_action` proof replay, validation |
