...
The plugin's home in your project is `.rn-agent/`. Saved actions, the UI skeleton, navigation graph, and runtime state all live there. `/setup` scaffolds the directory; [`/doctor`](/rn-dev-agent/commands/doctor/) reports on its health.
...
This walks all 16 prerequisite checks, retries failed installations, and gives manual instructions for anything it can't fix automatically. Once everything is green:
...
Want to watch it work? The [observability web UI](/rn-dev-agent/commands/observe/) autostarts with the session — run `/rn-dev-agent:observe` to get the URL and follow the live tool-call timeline, device mirror, and app-state panels in your browser.
