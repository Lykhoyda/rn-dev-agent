...
**maestro-runner** is auto-installed on first plugin load in Claude Code (via the SessionStart hook); on Codex, run `/rn-dev-agent:setup` once to install and verify it. Device control ships in-tree — the `rn-fast-runner` (iOS) and `rn-android-runner` (Android) runners build/install on first use; no external CLI is required. If anything is missing, run `/rn-dev-agent:setup` for guided diagnostics with manual fallback instructions.
...
This walks all 16 prerequisite checks, retries failed installations, and gives manual instructions for anything it can't fix automatically.
