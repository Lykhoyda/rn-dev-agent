...
**maestro-runner `1.1.24`** is installed into rn-dev-agent's versioned pin-cache on first plugin load in Claude Code (via its SessionStart hook). In Codex, invoke `$rn-dev-agent:setup`; after its read-only diagnosis, setup converges and verifies that exact runner before previewing project changes for consent. Replay never resolves from PATH, `~/.maestro-runner`, or brew Maestro. Device control ships in-tree through the iOS/Android runners. Use the matching host setup workflow for guided remediation.
