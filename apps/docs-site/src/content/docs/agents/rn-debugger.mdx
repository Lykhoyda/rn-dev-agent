...
Inspects `rn_session`, connects the exact target when needed, then gathers evidence in parallel:
...
| Metro bundle error | `cdp_metro_events` |
...
Confirms the exact session and target remain connected, takes a new screenshot,
verifies `cdp_component_tree` and `cdp_error_log` are clear, and re-runs the
failing action via Maestro.
