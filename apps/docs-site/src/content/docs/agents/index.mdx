---
title: Agents
description: 6 specialized agents — 3 spawnable for parallel codebase analysis, 3 parent-session-only for live device testing, PR QA, and debugging.
---

The plugin has **6 agents** split into two categories based on what tools they need.
...
| [`rn-pr-qa`](/rn-dev-agent/agents/rn-pr-qa/) | Opus | Device-test a GitHub PR on simulator, emulator, and/or physical device | `/rn-dev-agent:qa-pr` — runs inline |
...
- `/rn-dev-agent:qa-pr` — runs the `rn-pr-qa` protocol inline
