...
5. **Per selected target** (`ios`, `android`, `device`, or `all`): exact `bind_device` → reversible integration → managed `<pm> run ios|android` from the app root → `pin_dev_client` → feature proof: usable screen, recorded path, saved or reused Maestro action, return to the first screen before each off-camera rehearsal and the take, `device_record` start before the on-camera `cdp_run_action` proof replay, validation.
