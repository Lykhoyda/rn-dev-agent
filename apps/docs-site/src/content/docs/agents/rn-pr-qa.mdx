...
7. **Report.** The public GitHub comment is for a human reviewer of the feature that was built, not a plugin log. Missing hardware is SKIP with the exact remaining setup, not FAIL. Host screenshots and videos with `gh pr comment --attach`, video first, then rewrite screenshots to `<img … width="390">`. If the PR has `needs-qa`, run `gh pr edit "<pr-url>" --remove-label needs-qa`. A missing label is not FAIL.
