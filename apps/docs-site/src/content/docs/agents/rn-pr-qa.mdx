...
5. **Per selected target** (`ios`, `android`, `device`, or `all`): exact `bind_device` → reversible integration → managed `<pm> run ios|android` from the app root → `pin_dev_client` → feature proof: usable screen, recorded path, saved or reused Maestro action, off-camera rehearsal, `device_record` start before `maestro_run`, short take, validation.
