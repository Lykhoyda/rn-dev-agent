...
7. **Report** a per-target table. Missing hardware is SKIP with the exact remaining setup, not FAIL. Host screenshots and videos with `gh pr comment --attach`, video first, then rewrite screenshots to `<img … width="390">`.
