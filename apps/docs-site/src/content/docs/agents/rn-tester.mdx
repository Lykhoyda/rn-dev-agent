...
Checks `cdp_navigation_state` for auth-gated screens. If logged out, discovers and runs an owned login subflow through the exact pin-cache maestro-runner; missing owned automation is terminal rather than a manual-login fallback.
