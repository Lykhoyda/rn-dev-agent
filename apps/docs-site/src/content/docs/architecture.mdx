...
│  │  Tools across 6 families                        │ │
│  │  Fenced session authority on every live call    │ │
│  └─────────┬───────────────────────────┬───────────┘ │
│            │                            │             │
│  ┌─────────▼──────────┐      ┌─────────▼──────────┐ │
...
| **E2E testing** | maestro-runner `>= 1.1.24` (versioned pin-cache only; attested 1.1.24 default) | YAML-based persistent test files; underlying format for actions in `.rn-agent/actions/` |
...
The tools are organized into six families:
- **Session** — Bind one worktree, Metro, app, device, runner, Observe UI, and proof run (`rn_session`; see [parallel session authority](session-authority/))
