...
The agent must **understand intent and improvise on novel screens** without re-deriving journeys it has already verified. A login flow that took fourteen minutes to walk interactively the first time should become a deterministic, seconds-long prologue on later runs.
...
### Action reliability contract

| Situation | Required behaviour |
|---|---|
| `testID` renamed in app | `cdp_repair_action` patches the YAML via fuzzy match against the live snapshot, retries, and logs the diff |
| Button moved or restyled | Repair absorbs bounded UI drift; structural changes escalate |
| Product logic changed | The action refuses to auto-patch the logical break and surfaces the failure |
| Net-new behaviour to verify | The agent discovers it interactively, then **auto-saves the verified walk as a new action** |
| Repeated known journey | Replay uses the existing action so each verified flow compounds the reusable library |
