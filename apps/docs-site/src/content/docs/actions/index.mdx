...
Action replay runs only on maestro-runner `1.1.24` from rn-dev-agent's versioned pin-cache.
...
that exact version and `/doctor` reports missing, older, newer, checksum-mismatched,
...
Owned actions must carry `# enginePin: maestro-runner@1.1.24`. Setup migrates compatible
actions; missing or incompatible pins and unsupported regex text selectors are terminal.
