...
`-e KEY=VALUE` fills `${KEY}` placeholders inside the action. `--platform ios|android` validates the exact bound session platform and never selects among booted devices. `--dry-run` prints the resolved replay command without executing it.
...
Action replay runs on a pinned maestro-runner version (currently `1.0.9`). The pin lives in
`packages/rn-dev-agent-core/src/domain/engine-pin.ts` together with the engine's known quirks;
Replay results and `/doctor` report drift, so every session log records which
