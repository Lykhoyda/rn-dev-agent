...
| `cdp_login_prologue` | Enter authenticated journeys through the exact `user-login` action. Requires a fresh passing RunRecord and blocks unrelated mutations after failure; only its returned, authority-bound attach-only recovery may cross the latch. It is a navigation helper, not formal PR proof. |
