...
With the current storage layout, a new fenced session starts an action at
revision 1 with empty run and repair history and does not read an earlier
session's sidecar history or revision. Canonical lifecycle status remains
shared worktree knowledge:
when a clean replay promotes the tracked action YAML to `active`, later sessions
see that status without inheriting the earlier session's runtime history.
...
When a `testID` gets renamed in your app and the action references the old name, replay fails with `SELECTOR_NOT_FOUND`. The plugin doesn't give up: it looks at the live UI, finds the most likely new `testID` via fuzzy matching, patches the YAML, and retries. The repair gets logged to the runtime sidecar whose exact path is returned in `writes.runtimeStatePath`.
...
Action replay requires maestro-runner `>= 1.1.27` from rn-dev-agent's versioned pin-cache.
...
attested 1.1.27 as the default known-good and `/doctor` reports missing, older, unattested, checksum-mismatched,
...
Owned actions must carry `# enginePin: maestro-runner@1.1.27` or newer. Older or missing
pins refuse replay until you run setup or re-pin compatible actions with
`node <plugin-root>/rn-dev-agent-core/dist/maestro-runner-pin.js migrate-actions --root <app>`.
Unsupported regex text selectors require rewriting as literal text or `id:` before replay.
...
that the next step does not consume fails the replay. A `tapOn` whose exact target is not
...
Ownership is proven from the route scopes above the target: every ancestor carrying a route
record together with that route's own navigation must be focused, so a mounted-but-inactive
screen or a covered card's header refuses, and a scope the proof cannot read refuses as
`ASSERTION_FAILED`. Navigator chrome owns no route — a tab button is witnessed by the
descriptor it navigates to, so it stays pressable while its destination tab is inactive.
