...
revision 1 with empty run and repair history. Sidecar histories and revisions do
not carry over. Canonical lifecycle status remains shared worktree knowledge:
when a clean replay promotes the tracked action YAML to `active`, later sessions
see that status without inheriting the earlier session's runtime history. This
reset preserves isolation between parallel sessions and worktrees.
