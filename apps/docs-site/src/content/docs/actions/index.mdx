...
Maestro-certified active status. A `tapOn` whose exact target has no `onPress` first walks
up to the nearest pressable ancestor within eight fibers and presses it, so a `TextInput`
wrapped in a `Pressable` is tapped through the wrapper while the following `inputText`
still targets the exact input. When no such ancestor exists and the target is an editable
`TextInput`, the tap only designates that input for the immediately following `inputText`: it
