...
| `focused` | `boolean` | No |  |  | Type into the field that already has keyboard focus (software keyboard must be up) instead of binding an input. For Pressable-wrapped inputs whose inner TextInput is not in the snapshot. iOS only. ref still names the field you tapped and is used for the read-back. |
