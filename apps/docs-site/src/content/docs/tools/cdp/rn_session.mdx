...
|------|------|----------|---------|-------------|-------------|
| `action` | `enum: status | bind_device | bind_metro | pin_dev_client | prepare_handoff | cancel_handoff | accept_handoff | adopt_stale | release_stale_device | recover_arbiter | preview_integration | apply_integration | restore_integration | stop_metro | release` | Yes |  |  |  |
...
| `releaseHandle` | `unknown` | No |  |  | Bounded capability minted by bind_device for initial proven-dead device transfer |
...
```
rn_session(action: <enum: status | bind_device | bind_metro | pin_dev_client | prepare_handoff | cancel_handoff | accept_handoff | adopt_stale | release_stale_device | recover_arbiter | preview_integration | apply_integration | restore_integration | stop_metro | release>)
```
