...
Interact with React components by testID, accessibilityLabel, or supported discovery facts. Calls JS handlers directly, not native touch. typeText scans selector matches, accessibility names, hidden state, and self-or-descendant candidates under one shared, cycle-safe 2,000-work-unit limit; it normalizes each fiber to one preferred text handler, refuses incomplete or logically ambiguous resolution, and reports selector evidence from the source that owns the selected handler. It accepts testID, accessibilityLabel, placeholder, or role+name. accessibilityLabel matching uses exact, normalized, then substring tiers. setFieldValue uses a FormProvider only when it matches the nearest explicit control prop by identity, otherwise it safely matches that control to an ancestor useForm hook return before calling setValue. Portal roots can be registered through globalThis.__RN_AGENT_EXTRA_ROOTS__. walkUp (opt-in): for action:"press" with testID/accessibilityLabel selectors only, walks up at most 8 fiber ancestors to the nearest pressable and refuses absence or ambiguity. Use device_swipe/device_press for native gestures.
