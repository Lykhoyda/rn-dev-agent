---
title: MCP Tools
description: MCP tools for fenced sessions, CDP introspection, device interaction, actions, testing, and macro-asserts.
---

The MCP server exposes tools organized into six conceptual families. All tools are available through Claude Code, Codex, and Cursor.

## When to use which family

| Need | Use | Example |
|------|-----|---------|
| Inspect or transition the fenced worktree session | **Session** | `rn_session` |
...
| Check environment authority and health | **Session**, then **CDP** | `rn_session(action="status")`, then passive `cdp_status` |
| Debug crashes spanning JS + native | **CDP** | `collect_logs` with multiple sources |

## Session Tool

`rn_session` passively reports status or performs explicit, fail-closed device,
Metro, integration, handoff, adoption, recovery, and release transitions. See
[Parallel session authority](../session-authority/) for the lifecycle and threat
boundary.
...
**Connection lifecycle:** `cdp_status` (passive diagnostics) → `cdp_connect` (pin the exact authority-bound target) → `cdp_disconnect` (teardown) → `cdp_targets` (diagnostic list without connecting) → `cdp_restart` (full process restart) → `cdp_open_devtools` (attach DevTools)
...
`maestro_run` / `maestro_generate` → `maestro_test_all` → `proof_step` → `cross_platform_verify`. `cdp_auto_login` remains a legacy per-call helper outside the guarded journey; it is not a fallback from a failed `cdp_login_prologue` replay, and it is not PR proof.
